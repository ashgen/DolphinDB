...
j < 5; j++) {
time->
set(j, Util::createDateTime(j * 100000)
);
}
arrayVector->
append(time);
...
cout << "symbol vector: " << time << "ms" <<
endl;

conn.run("undef(all)");
conn.
run(getSymbolVector("v", 2000000)
);
startTime = getTimeStampMs();
conn.run("v");
time = getTimeStampMs() - startTime;
cout << "symbol vector optimize:" << time << "ms" <<
endl;
...
conn.run("t");
time = getTimeStampMs() - startTime;
cout << "table with one symbol vector:" << time << "ms" <<
endl;
...
conn.run("t");
time = getTimeStampMs() - startTime;
cout << "table with one symbol vector optimize:" << time << "ms" <<
endl;
...
conn.run("t");
time = getTimeStampMs() - startTime;
cout << "table with same symbol vectors:" << time << "ms" <<
endl;
...
conn.run("t");
time = getTimeStampMs() - startTime;
cout << "table with diff symbol vectors:" << time << "ms" <<
endl;
...
conn.run("t");
time = getTimeStampMs() - startTime;
cout << "tuple symbol tuple:" << time << "ms" <<
endl;
...
conn.
upload(mixtimename, mixtimedata
);
}
...
EXPECT_EQ(table
->getColumn(0)->getString(0), string("1"));
EXPECT_EQ(table
->getColumn(1)->getString(0), string("20000"));
}
...
EXPECT_EQ(dict
->
get(Util::createInt(1)
)->

getString(), string("IBM")

);
EXPECT_EQ(dict
->
get(Util::createInt(2)
)->

getString(), string("MSFT")

);
EXPECT_EQ(dict
->
get(Util::createInt(3)
)->

getString(), string("GOOG")
...
vector <string> testValues{"9223372036854775807", "helloworldabcdefghijklmnopqrstuvwxyz", "智臾科技a",
                           "hello,智臾科技a", "123abc您好！a", ""};
int buckets[5] = {13, 43, 71, 97, 4097};
int expected[5][6] = {
        {8,   1,    9,    3,    3,   0},
        {37,  20,   25,   25,   27,  0},
...
i++)
v->
append(Util::parseConstant(DT_UUID, t->getColumn(0)->getString(i))
);
for (
unsigned int j = 0;
j < 5; j++) {
...
i++)
v->
append(Util::parseConstant(DT_IP, t->getColumn(0)->getString(i))
);
for (
unsigned int j = 0;
j < 5; j++) {
...
i++)
v->
append(Util::parseConstant(DT_INT128, t->getColumn(0)->getString(i))
);
for (
unsigned int j = 0;
j < 5; j++) {
...
vector<char> testValues{127, -127, 12, 0, -12, -128};
int buckets[5] = {13, 43, 71, 97, 4097};
int expected[5][6] = {{10,  12,  12, 0, 10,  -1},
                      {41,  18,  12, 0, 4,   -1},
                      {56,  24,  12, 0, 68,  -1},
                      {30,  5,   12, 0, 23,  -1},
                      {127, 129, 12, 0, 244, -1}};
...
vector<short> testValues{32767, -32767, 12, 0, -12, -32768};
int buckets[5] = {13, 43, 71, 97, 4097};
int expected[5][6] = {{7,    2,   12, 0, 10,  -1},
                      {1,    15,  12, 0, 4,   -1},
                      {36,   44,  12, 0, 68,  -1},
                      {78,   54,  12, 0, 23,  -1},
                      {4088, 265, 12, 0, 244, -1}};
...
vector <DATA_TYPE> colTypes = {DT_SYMBOL, DT_SYMBOL, DT_SYMBOL, DT_SYMBOL, DT_SYMBOL, DT_SYMBOL, DT_SYMBOL, DT_SYMBOL,
                               DT_SYMBOL, DT_SYMBOL};
...
i++) {
columnVecs[0]->
set(i, Util::createString("A" + std::to_string(i % 1000))
);
columnVecs[1]->
set(i, Util::createString("B" + std::to_string(i % 1000))
);
columnVecs[2]->
set(i, Util::createString("C" + std::to_string(i % 1000))
);
columnVecs[3]->
set(i, Util::createString("D" + std::to_string(i % 1000))
);
columnVecs[4]->
set(i, Util::createString("E" + std::to_string(i % 1000))
);
columnVecs[5]->
set(i, Util::createString("F" + std::to_string(i % 1000))
);
columnVecs[6]->
set(i, Util::createString("G" + std::to_string(i % 1000))
);
columnVecs[7]->
set(i, Util::createString("H" + std::to_string(i % 1000))
);
columnVecs[8]->
set(i, Util::createString("I" + std::to_string(i % 1000))
);
columnVecs[9]->
set(i, Util::createString("J" + std::to_string(i % 1000))
);
}
int64_t startTime, time;
startTime = getTimeStampMs();
