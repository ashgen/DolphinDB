namespace {
    ...
    static vector <string> type = {"BOOL", "CHAR", "SHORT", "STRING", "LONG", "NANOTIME", "NANOTIMESTAMP", "TIMESTAMP",
                                   "FLOAT", "DOUBLE", "INT", "DATE", "MONTH", "TIME", "SECOND", "MINUTE", "DATETIME",
                                   "DATEHOUR", "IPADDR", "INT128", "SYMBOL"};
    ...
    cout << "test_batchTableWriter_insertTo_inMemoryTable" <<
    endl;
    EXPECT_ANY_THROW(btw
    .insert("table1", "", int(0), int(0), short(100), strtest, string("test"), testlong, testlong, testlong, testlong, float(100), double(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100)));
    EXPECT_ANY_THROW(btw
    .insert("table1", "", char('0'), char('t'), short(100), strtest, string("test"), testlong, testlong, testlong, testlong, float(100), double(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100), int(100)));
...
{
while (btw.getAllStatus()->getColumn(3)->getString(0).
...
cout << "now rows:" << btw.getAllStatus()->getColumn(3)->getString(0).
...
ASSERT_EQ(btw
.getAllStatus()->getColumn(3)->getInt(0), 1000);
}
int result1 = 0;
for (
int i = 0;
...
ASSERT_EQ(btw
.getAllStatus()->getColumn(3)->getInt(0), 3000);
}
result1 = 0;
for (
int i = 0;
...
cout << "test_batchTableWriter_insertTo_partitionedTable" <<
endl;
//size1000
...
i < 1000; i++) {
string script =
        "dt=loadTable('dfs://test_batchTableWriter', 'dtable'); eqObj(( select * from dt)[" + std::to_string(i) +
        "].values(),expected[0].values());";
result2 += conn.
run(script)
->
...
cout << "test_batchTableWriter_insertTo_partitionedTable_2" <<
endl;
//size1000
...
i < 1000; i++) {
string script =
        "pt=loadTable('dfs://test_batchTableWriter', 'ptable'); eqObj(( select * from pt)[" + std::to_string(i) +
        "].values(),expected[0].values());";
result3 += conn.
run(script)
->
...
ASSERT_EQ(btw
.getAllStatus()->getColumn(3)->getInt(0), 3000000);
string script2;
...
int i = 0;
i < 3000000; i++) {
btw.insert("dfs://test_batchTableWriter_symbol", "pt", "A" +
std::to_string(i
% 10), "B" +
std::to_string(i
% 10), "C" +
...
ASSERT_EQ(btw
.getAllStatus()->getColumn(3)->getInt(0), 10);
btw.removeTable("st");
EXPECT_EQ(1, 1);
}
...
ASSERT_EQ(btw
.getAllStatus()->getColumn(3)->getInt(0), 65536);
...
EXPECT_ANY_THROW(get<0>(btw.getStatus("test_remove", ""))
);// table has been removed, so get one exception.
}
...
static void test_batchTableWriter_insert_thread_fuction_using_cpp_type(int id, BatchTableWriter &btw, string dbName,
                                                                       string tableName,
                                                                       TableSP data) {
    DBConnection connNew;
    connNew.connect(hostName, port, "admin", "123456");
...
static void
test_batchTableWriter_insert_multithread_using_CPP_type(BatchTableWriter &btw, string dbName, string tableName,
                                                        bool partitionTable,
                                                        string assertStr, int testRow) {
    string script;
    DBConnection connNew(false, false);
...
BatchTableWriter btw(hostName, port, "admin", "123456", true);
test_batchTableWriter_insert_multithread_using_CPP_type(ref(btw),
...
BatchTableWriter btw(hostName, port, "admin", "123456", true);
test_batchTableWriter_insert_multithread(ref(btw),
"dfs://batchTableWriter", "batchTableWriter", true,
...
BatchTableWriter btw(hostName, port, "admin", "123456", true);
test_batchTableWriter_insert_multithread(ref(btw),
"dfs://batchTableWriter", "batchTableWriter", false,
...
i < 4; ++i) {
threadVec[i] =
thread(test_batchTableWriter_insert_multithread, ref(btw),
"dfs://batchTableWriter" +
to_string(i),
"batchTableWriter" +
to_string(i),
true,
...
"Failed to get queue depth. Please use addTable to add infomation of database and table first.");
}
}
...
static void test_BatchTableWriter_create_network() {
    EXPECT_ANY_THROW(BatchTableWriter
    btw(nullptr, port, "admin", "123456", true));
    EXPECT_ANY_THROW(BatchTableWriter
    btw(hostName, port, nullptr, "123456", true));
    EXPECT_ANY_THROW(BatchTableWriter
    btw(hostName, port, "admin", nullptr, true));
    BatchTableWriter btw(hostName, 0, "admin", "123456", true);//no exception
}
...
static void test_BatchTableWriter_insert_network() {
    ...
    string res = conn.run(
            "exec * from loadTable(\"dfs://test_batchTableWriter\",\"test_BatchTableWriter_insert_network\")")->values()->getString();
    EXPECT_EQ(res, "([1],[1])");
    btw.removeTable(dbName, tableName);
}
...
cout << "PASSED--test_BatchTableWriter_creat_network" <<
endl;

test_BatchTableWriter_addTable_network();

cout << "PASSED--test_BatchTableWriter_addTable_network" <<
endl;

test_BatchTableWriter_remove_exception();

cout << "PASSED--test_BatchTableWriter_remove_exception" <<
endl;

test_BatchTableWriter_insert_network();

cout << "PASSED--test_BatchTableWriter_insert_network" <<
endl;

test_BatchTableWriter_remove_and_insert();

cout << "PASSED--test_BatchTableWriter_remove_and_insert" <<
endl;
}

static void BatchTableWriter_insert_error_type(string
destType,
BatchTableWriter &btw
) {
if (destType == "BOOL" || destType == "CHAR") {
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", short(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", long(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", (long long) (1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", int(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", double(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", float(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", to_string(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", to_string(1).
...
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createShort(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createLong(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createInt(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createDouble(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createFloat(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createDate(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createMonth(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createTime(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createMinute(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createSecond(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createDateTime(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createTimestamp(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createNanoTime(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createNanoTimestamp(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createString("1")));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", Util::createDateHour(1)));
} else if (destType == "SHORT") {
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", bool(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", char(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", long(1)));
EXPECT_ANY_THROW(btw
.insert("batchTableWriter", "", (long long) (1)));
