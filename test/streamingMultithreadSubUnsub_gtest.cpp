void pubTables2(const vector <string> tables) {
    for (unsigned int i = 0; i < tables.size(); i++) {
        ...
        string script =
                ...
                t + ",true,true,10000000);\
				def writeData(t){\
					index = 0;\
					do{\
						num = rand(3..6,1)[0];\
						insert into " +
                t + " values(index..(index + num - 1) , 1..num );\
						sleep(num * 100);\
						index += num;\
					}while(true);\
				};\
				submitJob('ss','ss',writeData{" +
                t + "})";
        conn.run(script);
    }
}
...
i++) {
ts.
push_back(std::thread(subScriteTableTest3, hostName, port, listenPorts[i], tables[i])
);
    }
