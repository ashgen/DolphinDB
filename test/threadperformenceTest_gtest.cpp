...
mulwrite = new MultithreadedTableWriter(hostName, port, "admin", "123456", dbName, "pt", NULL, false, NULL, 1000, 1, 1,
                                        "date");
...
cout << "Test_MultithreadedTableWriter_performence_threadCount_1-> Time expended: " << runTime << "s" <<
endl;
}
...
if (result->getColumn(3)->getString(0).
...
runTime = (tm_end.tv_sec - tm_start.tv_sec) + (double) (tm_end.tv_usec - tm_start.tv_usec) / 1000000;
cout << "Test_batchTableWriter_performence-> Time expended: " << runTime << "s" <<
endl;
            break;
        }
} while (true);
