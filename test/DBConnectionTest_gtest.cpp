...
if (https_conf == false) {
cout << "skip this case when server's 'enableHTTPS' is False." <<
endl;
EXPECT_EQ(1, 1);
} else {
...
cout << conn_demo.run("getRecentJobs()")->
...
if (v.find("2.10") != 0) {
cout << "server version not matched: 2.10.xx, skip this case." <<
endl;
EXPECT_EQ(1, 1);
} else {
...
cout << "check passed..." <<
endl;
cout << "check if reconnected..." <<
endl;
EXPECT_EQ(connReconn
.run("1+1")->
...
EXPECT_EQ(1000, ex_tab->

rows()

);
EXPECT_FALSE(pool_demo
.
...
connReconn.
run(srcipt1);
EXPECT_ANY_THROW(pool_demo
.run("for(i in 1..100){tableInsert(tmp,rand(100,1),rand(100,1),rand(100,1));};select * from tmp", -1));
