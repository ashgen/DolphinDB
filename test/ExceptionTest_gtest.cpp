...
const std::exception &e
        ...
        std::cerr << e.what() << '\n';
string ex_str =
        "Incompatible type. Expected: " + Util::getDataTypeString(dt1) + ", Actual: " + Util::getDataTypeString(dt2);
EXPECT_STREQ(e
.
...
vector <IO_ERR> ioErrVec = {OK, DISCONNECTED, NODATA, NOSPACE, TOO_LARGE_DATA, INPROGRESS, INVALIDDATA, END_OF_STREAM,
                            READONLY, WRITEONLY, NOTEXIST, CORRUPT, NOT_LEADER, OTHERERR};
    string getMsg;
for (
int i = 0;
i<ioErrVec.
...
("Testing case " + eCase + "_" + esubCase + " failed").
