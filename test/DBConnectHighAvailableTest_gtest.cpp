...
i++) {
columnVecs[0]->
set(i, Util::createString("stu_" + to_string(rand() % rowNum))
);
columnVecs[1]->
set(i, Util::createInt(rand() % rowNum)
);
columnVecs[2]->
set(i, Util::createDateTime(rand() % rowNum)
);
    }
...
Signal notify;
    ...
    auto onehandler = [&](Message msg) {
        ...
        cout << "current datanode: " + conn.run("getNodeAlias()")->getString() << endl
             << endl;
        if (msg_total == 2000) {
            notify.set();
        }
...
cout << "total insert rows are " + conn.run("exec count(*) from loadTable(\"dfs://test_dfs_HA\", `pt_dfs_HA)")->

getString()

<<
endl;
cout << "Now datanode has changed to " + conn.run("getNodeAlias()")->
