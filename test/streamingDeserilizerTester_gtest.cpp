...
Signal notify;
    ...
    auto onehandler = [&](Message msg) {
        ...
        if (symbol == "msg1") {
            msg1_total += 1;
            EXPECT_EQ(msg->get(0)->getString(),
                      table1_SDPT->getRow(index1)->get(Util::createString("datetimev"))->getString());
            EXPECT_EQ(msg->get(1)->getString(),
                      table1_SDPT->getRow(index1)->get(Util::createString("timestampv"))->getString());
            EXPECT_EQ(msg->get(2)->getString(),
                      table1_SDPT->getRow(index1)->get(Util::createString("sym"))->getString());
            EXPECT_EQ(msg->get(3)->getString(),
                      table1_SDPT->getRow(index1)->get(Util::createString("price1"))->getString());
            EXPECT_EQ(msg->get(4)->getString(),
                      table1_SDPT->getRow(index1)->get(Util::createString("price2"))->getString());
            index1++;
        } else if (symbol == "msg2") {
            msg2_total += 1;
            EXPECT_EQ(msg->get(0)->getString(),
                      table2_SDPT->getRow(index2)->get(Util::createString("datetimev"))->getString());
            EXPECT_EQ(msg->get(1)->getString(),
                      table2_SDPT->getRow(index2)->get(Util::createString("timestampv"))->getString());
            EXPECT_EQ(msg->get(2)->getString(),
                      table2_SDPT->getRow(index2)->get(Util::createString("sym"))->getString());
            EXPECT_EQ(msg->get(3)->getString(),
                      table2_SDPT->getRow(index2)->get(Util::createString("price1"))->getString());
            index2++;
        }
...
auto thread1 = threadedClient.subscribe(hostName, port, onehandler, "SDoutTables", "test_SD", 0, true, nullptr, false,
                                        false, "admin", "123456", sdsp);
...
Signal notify;
    ...
    ThreadedClient threadedClient(listenport);
auto thread2 = threadedClient.subscribe(hostName, port, batchhandler, "SDoutTables", "mutiSchemaBatch", 0, true,
                                        nullptr, true, 1, 1.0, false, "admin", "123456", sdsp);
...
auto queue = client.subscribe(hostName, port, "SDoutTables", "actionTest", 0, true, nullptr, false, false, "admin",
                              "123456", sdsp);
...
getString(), table1_SDPT

->
getRow(index1)
->
get(Util::createString("datetimev")
)->
...
getRow(index1)
->
get(Util::createString("timestampv")
)->
...
auto threadVec = client.subscribe(hostName, port, onehandler, "SDoutTables", "test_SD", 0, true, nullptr, false, false,
                                  "admin", "123456", sdsp);
EXPECT_EQ(threadVec
.
...
EXPECT_ANY_THROW(
        auto thread1 = threadedClient.subscribe(hostName, port, onehandler, "SDoutTables", "test_SD", 0, true, nullptr,
                                                true, false, "admin", "123456", sdsp));
...
EXPECT_ANY_THROW(
        auto queue = client.subscribe(hostName, port, "SDoutTables", "actionTest", 0, true, nullptr, true, false,
                                      "admin", "123456", sdsp));
...
EXPECT_ANY_THROW(
        auto threadVec = client.subscribe(hostName, port, onehandler, "SDoutTables", "test_SD", 0, true, nullptr, true,
                                          false, "admin", "123456", sdsp));
