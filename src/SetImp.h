...
namespace dolphindb {
    ...
    public:
        ...
        AbstractSet(DATA_TYPE type, const unordered_set<T> &data) : type_(type), category_(Util::getCategory(type_)),
                                                                    data_(data) {}
        ...
        virtual DATA_TYPE getRawType() const {
            return type_ == DT_SYMBOL ? DT_INT : Util::convertToIntegralDataType(type_);
        }
