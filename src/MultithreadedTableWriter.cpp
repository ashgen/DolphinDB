...
#define DLOG true ? DLogger::GetMinLevel() : DLogger::Info
    ...
    MultithreadedTableWriter::MultithreadedTableWriter(const std::string &hostName, int port, const std::string &userId,
                                                       const std::string &password,
                                                       const string &dbName, const string &tableName, bool useSSL,
                                                       bool enableHighAvailability,
                                                       const vector<string> *pHighAvailabilitySites,
                                                       int batchSize, float throttle, int threadCount,
                                                       ...
                                                       const std::function<void(ConstantSP)> &callbackFunc) : dbName_(
            dbName),
                                                                                                              tableName_(
                                                                                                                      tableName),
                                                                                                              batchSize_(
            ...
            if (isPartionedTable_) {
                ...
                if (partColNames->isScalar()) {
                    if (partColNames->getString() != partitionCol) {
                        throw RuntimeException("The parameter partionCol must be the partitioning column '" +
                                               partColNames->getString() + "' in the partitioned table");
                    }
                    ...
                    if (index < 0)
                        throw RuntimeException(
                                "The parameter partionCol must be the partitioning columns in the partitioned table");
                ...
                partitionDomain_ = Util::createDomain((PARTITION_TYPE) partitionType, partitionColType,
                                                      partitionSchema);
        ...
        if (mode == M_Append) {
            if (dbName_.empty()) {//memory table
                ...
                scriptTableInsert_ = std::move(
                        std::string("tableInsert{loadTable(\"") + dbName_ + "\",\"" + tableName_ + "\")}");
            ...
            if (dbName_.empty()) {//memory table
                ...
                scriptTableInsert_ = std::move(
                        std::string("upsert!{loadTable(\"") + dbName_ + "\",\"" + tableName_ + "\")");
            } else {//single partitioned table
                scriptTableInsert_ = std::move(
                        std::string("upsert!{loadTable(\"") + dbName_ + "\",\"" + tableName_ + "\")");
            }
    ...
    bool
    MultithreadedTableWriter::insert(std::vector<ConstantSP> **records, int recordCount, ErrorCodeInfo &errorInfo) {
        if (hasError_.load()) {
    ...
    */
    ...
    void MultithreadedTableWriter::callBack(std::function<void(ConstantSP)> callbackFunc, bool result,
                                            vector<vector<ConstantSP> *> &queue1) {
        ...
        while (isExit() == false) {
            {
                ...
                if (tableWriter_.batchSize_ > 1 && tableWriter_.throttleMilsecond_ > 0) {
                    ...
                    while (isExit() == false &&
                           writeThread_.writeQueue.size() < tableWriter_.batchSize_) {//check batchsize
    ...
    bool MultithreadedTableWriter::SendExecutor::writeAllData() {
        ...
        try {
            ...
            {//create table
                writeTable = Util::createTable(tableWriter_.saveColNames_, tableWriter_.saveColTypes_, 0, size,
                                               tableWriter_.saveColExtras_);
                ...
                for (INDEX saveCol = 0; saveCol < colSize; saveCol++) {
                    ...
                    for (int i = 0; i < size; i++, pcurRow++) {
                        if (pcol->append((*pcurRow)->at(itemColIndex)) == false) {
                            throw RuntimeException(
                                    "insert " + (*pcurRow)->at(itemColIndex)->getString() + " to column " +
                                    tableWriter_.saveColNames_[saveCol] + " failed.");
                        }
                    }
                }
                addRowCount += size;
