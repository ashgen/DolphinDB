...
namespace dolphindb {
    ...
    TableSP SharedMemStream::readData(TableSP &copy, const bool &overwrite, size_t &readRows, int &microCost) {
        ...
        if (shmpHeader_->writePage_ == shmpHeader_->readPage_ &&
            shmpHeader_->readOffset_ == shmpHeader_->writeOffset_) {
        ...
        if ((shmpHeader_->writePage_ > shmpHeader_->readPage_) && (dataHeader->length_ == 0 ||
                                                                   shmpHeader_->writeOffset_ >
                                                                   shmpHeader_->readOffset_)) {
        ...
        if (!copy.isNull()) {
            if (dataHeader->rows_ + startRows > copy->size()) {
                for (int colIndex = 0; colIndex < copy->columns(); colIndex++) {
                    if (copy->getColumnType(colIndex) == DT_STRING || copy->getColumnType(colIndex) == DT_SYMBOL ||
                        copy->getColumnType(colIndex) == DT_BLOB) {
                        continue;
                    }
            ...
            table = Util::createTable(tableMetaIt->second.colNames_, tableMetaIt->second.colTypes_, dataHeader->rows_,
                                      dataHeader->rows_);
        }

        for (unsigned int i = 0; i < tableMetaIt->second.colLength_.size(); i++) {
            VectorSP colToWrite = table->getColumn(i);
            if (table->getColumnType(i) == DT_STRING || table->getColumnType(i) == DT_SYMBOL ||
                table->getColumnType(i) == DT_BLOB) {
    ...
    IPCInMemTable::IPCInMemTable(bool create, std::string shmPath, std::string tableName,
                                 const vector<ConstantSP> &cols,
                                 const vector<string> &colNames, size_t bufSize) : BasicTable(cols, colNames),
