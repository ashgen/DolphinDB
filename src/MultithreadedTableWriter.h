...
namespace dolphindb {
     ...
     */
        MultithreadedTableWriter(const std::string &host, int port, const std::string &userId,
                                 ...
                                 int batchSize = 1, float throttle = 0.01f, int threadCount = 1,
        ...
        bool insert(ErrorCodeInfo &errorInfo, TArgs... args) {
            ...
            }
            {
                ...
                if (argSize != colTypes_.size()) {
                    errorInfo.set(ErrorCodeInfo::EC_InvalidParameter,
                                  "Column counts don't match " + std::to_string(argSize));
                    return false;
                }
            }
            {
                ...
                ConstantSP result[] = {
                        Util::createObject(getColDataType(colIndex1++), args, &errorInfo, colExtras_[colIndex2++])...};
        ...
        bool insertUnwrittenData(std::vector<std::vector<ConstantSP> *> &records, ErrorCodeInfo &errorInfo) {
            return insert(records.data(), records.size(), errorInfo);
        }
    ...
    private:
        ...
        static void
        callBack(std::function<void(ConstantSP)> callbackFunc, bool result, vector<vector<ConstantSP> *> &queue1);
        ...
        public:
            SendExecutor(MultithreadedTableWriter &tableWriter, WriterThread &writeThread) : tableWriter_(tableWriter),
                                                                                             writeThread_(
                                                                                                     writeThread) {};
