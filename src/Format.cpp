...
namespace dolphindb {
    ...
    const char *TemporalFormat::monthName[12] = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT",
                                                 "NOV", "DEC"};
    vector<pair<int, int>> TemporalFormat::formatMap;
    const long long NumberFormat::power10_[10] = {10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000,
    ...
    NumberFormat::NumberFormat(const string &format) : percent_(0), science_(0), segmentLength_(INT_MAX),
                                                       integerMinDigits_(0), fractionMinDigits_(0),
                                                       fractionOptionalDigits_(0), headSize_(0), tailSize_(0),
                                                       rounding_(0) {
        initialize(format);
    }
    ...
    void NumberFormat::initialize(const string &format) {
            ...
            if (ch == '#' || ch == '.' || ch == '0' || ch == ',' || ch == 'E' || ch == '%') {
                if (firstSymPos < 0)
                    ...
                    throw RuntimeException(
                            "Characters other than 0/#/./,/E/% can't appear in the middle of a number format.");
                lastSymPos = i;
                if (ch == 'E') {
                    sciencePos = i;
                    scienceCount++;
        ...
        if (sciencePos >= 0) {
            if (sciencePos == firstSymPos || sciencePos == lastSymPos)
                throw RuntimeException(
                        "The scientific notation(E) can't be the first symbol or last symbol of a number format.");
        ...
        if (commaPos >= 0) {
            if (commaPos == firstSymPos || commaPos == lastSymPos)
                throw RuntimeException(
                        "The decimal separator(,) can't be the first symbol or last symbol of a number format.");
            if (pointPos >= 0) {
                if (pointPos < commaPos)
                    throw RuntimeException(
                            "The decimal separator(,) must appear before decimal point(.) in a number format.");
        ...
        for (int i = 0; i <= len; ++i) {
            if (i == len || (i != 0 && (format_[i] != format_[i - 1] || escape[i] != escape[i - 1]))) {
                ...
                if (ch >= 0 && !escape[i - 1] && formatMap[ch].first != -1) {
                    segments_[segmentCount_++] = FormatSegment(formatMap[ch].first, formatMap[ch].second, i - cnt,
                                                               i - 1);
