 ...
 */
    string IPAddr::toString(const unsigned char *data) {
        ...
        if (isIP4) {
            ...
            for (int i = 0; i < 16; i = i + 2) {
                if (i > 0 && i < 12 && !consecutiveZeros &&
                    *(unsigned int *) (data + (Util::LITTLE_ENDIAN_ORDER ? 11 - i : i)) == 0) {
                    //consecutive zeros
    ...
    DateTime::DateTime(int year, int month, int day, int hour, int minute, int second) : TemporalScalar(
    ...
    Timestamp::Timestamp(int year, int month, int day, int hour, int minute, int second, int milliSecond) : Long(
            countTemporalUnit(Util::countDays(year, month, day), 86400000ll,
    ...
    NanoTimestamp::NanoTimestamp(int year, int month, int day, int hour, int minute, int second, int nanoSecond) : Long(
            countTemporalUnit(Util::countDays(year, month, day), 86400000000000ll,
    ...
    DateHour::DateHour(int year, int month, int day, int hour) : TemporalScalar(
            countTemporalUnit(Util::countDays(year, month, day), 24, hour)) {
    }
    ...
    ConstantSP DateTime::castTemporal(DATA_TYPE expectType) {
        ...
        if (expectType == DT_NANOTIMESTAMP || expectType == DT_TIMESTAMP) {
            ...
            val_ == INT_MIN ? result = LLONG_MIN : result =
                                                           (long long) (remainder + ((val_ < 0) && remainder) * 86400) *
                                                           1000000000LL;
            return Util::createObject(expectType, result);
        } else {
            ratio = Util::getTemporalConversionRatio(DT_SECOND, expectType);
    ...
    ConstantSP Timestamp::castTemporal(DATA_TYPE expectType) {
        ...
        if (expectType == DT_NANOTIMESTAMP) {
            ...
            val_ == LLONG_MIN ? result = LLONG_MIN : result = (remainder + ((val_ < 0) && remainder) * 86400000) *
                                                              1000000ll;
            return Util::createObject(expectType, result);
        } else {
    ...
    ConstantSP NanoTimestamp::castTemporal(DATA_TYPE expectType) {
        ...
        if (expectType == DT_TIMESTAMP) {
            ...
            long long remainder = val_ % 86400000000000ll;
            val_ == LLONG_MIN ? result = INT_MIN : result = (remainder + (val_ < 0 && remainder) * 86400000000000ll) /
                                                            ratio;
            return Util::createObject(expectType, result);
        }
    }
