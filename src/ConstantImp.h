...
namespace dolphindb {
    ...
    public:
        ...
        AnyVector(const vector<ConstantSP> &data, bool containNull) : data_(data.begin(), data.end()),
        ...
        virtual const string &getStringRef(INDEX index) const {
            throw RuntimeException("getStringRef method not supported for AnyVector");
        }
        ...
        virtual int
        serialize(char *buf, int bufSize, INDEX indexStart, int offset, int &numElement, int &partial) const {
    ...
    public:
        AbstractFastVector(int size, int capacity, T *srcData, T nullVal, bool containNull) : nullVal_(nullVal),
                                                                                              size_(size),
                                                                                              capacity_(capacity),
                                                                                              containNull_(
                                                                                                      containNull) {
        ...
        virtual ConstantSP getSubVector(INDEX start, INDEX length, INDEX capacity) const {
            ...
            if (data)
                return Util::createVector(type, std::abs(length), capacity, true, getExtraParamForType(), data,
                                          containNull_);
            else
        ...
        int serialize(char *buf, int bufSize, INDEX indexStart, int offset, int cellCountToSerialize, int &numElement,
                      int &partial) const {
            //assume offset==0 and bufSize>=sizeof(T)
            if (indexStart >= size_)
                return -1;
    ...
    public:
        FastBoolVector(int size, int capacity, char *srcData, bool containNull) : AbstractFastVector(size, capacity,
                                                                                                     srcData, CHAR_MIN,
                                                                                                     containNull) {
            dataType_ = DT_BOOL;
        }
    ...
    public:
        FastCharVector(int size, int capacity, char *srcData, bool containNull) : AbstractFastVector(size, capacity,
                                                                                                     srcData, CHAR_MIN,
                                                                                                     containNull) {
            dataType_ = DT_CHAR;
        }
    ...
    public:
        FastShortVector(int size, int capacity, short *srcData, bool containNull) : AbstractFastVector(size, capacity,
                                                                                                       srcData,
                                                                                                       SHRT_MIN,
                                                                                                       containNull) {
            dataType_ = DT_SHORT;
    ...
    public:
        FastIntVector(int size, int capacity, int *srcData, bool containNull) : AbstractFastVector(size, capacity,
                                                                                                   srcData, INT_MIN,
                                                                                                   containNull) {
            dataType_ = DT_INT;
        }
    ...
    public:
        FastLongVector(int size, int capacity, long long *srcData, bool containNull) : AbstractFastVector(size,
                                                                                                          capacity,
                                                                                                          srcData,
                                                                                                          LLONG_MIN,
                                                                                                          containNull) {
    ...
    public:
        FastFloatVector(int size, int capacity, float *srcData, bool containNull) : AbstractFastVector(size, capacity,
                                                                                                       srcData,
                                                                                                       FLT_NMIN,
                                                                                                       containNull) {
            dataType_ = DT_FLOAT;
    ...
    public:
        FastDoubleVector(int size, int capacity, double *srcData, bool containNull) : AbstractFastVector(size, capacity,
                                                                                                         srcData,
                                                                                                         DBL_NMIN,
                                                                                                         containNull) {
            dataType_ = DT_DOUBLE;
    ...
    public:
        FastTemporalVector(int size, int capacity, int *srcData, bool containNull) : FastIntVector(size, capacity,
                                                                                                   srcData,
                                                                                                   containNull) {}
    ...
    public:
        FastArrayVector(int size, int capacity, char *srcData, bool containNull, DATA_TYPE dataType,
                        INDEX *pindex = NULL, int extraParam = 0) {
        ...
        FastArrayVector(const VectorSP &index, const VectorSP &value) {
            ...
            if (!index->isFastMode())
                throw RuntimeException(
                        "The index vector for the constructor of FastArrayVector must be a regular vector.");
        ...
        virtual bool validIndex(INDEX start, INDEX length, INDEX uplimit) {
            return value_->validIndex(start, length, uplimit);
        }
        ...
        virtual int compare(INDEX index, const ConstantSP &target) const {
            throw RuntimeException("Array Vector doesn't support method compare");
        }
        ...
        virtual bool getHash(INDEX start, int len, int buckets, int *buf) const {
            throw RuntimeException("Array Vector doesn't support method getHash");
        }
        ...
        INDEX
        checkVectorSize();//<0: The vectors in the array vector are of different size. >=0: The vectors are of the same size {x}.
    ...
    private:
        int serializeFixedLength(char *buf, int bufSize, INDEX indexStart, int offset, int targetNumElement,
                                 int &numElement, int &partial) const;

        int serializeVariableLength(char *buf, int bufSize, INDEX indexStart, int offset, int targetNumElement,
                                    int &numElement, int &partial) const;

        IO_ERR deserializeFixedLength(DataInputStream *in, INDEX indexStart, INDEX targetNumElement, INDEX &numElement);

        IO_ERR
        deserializeVariableLength(DataInputStream *in, INDEX indexStart, INDEX targetNumElement, INDEX &numElement);
    ...
    public:
        FastDateVector(int size, int capacity, int *srcData, bool containNull) : FastTemporalVector(size, capacity,
                                                                                                    srcData,
                                                                                                    containNull) {
            dataType_ = DT_DATE;
        }
    ...
    public:
        FastDateTimeVector(int size, int capacity, int *srcData, bool containNull) : FastTemporalVector(size, capacity,
                                                                                                        srcData,
                                                                                                        containNull) {
    ...
    public:
        FastDateHourVector(int size, int capacity, int *srcData, bool containNull) : FastTemporalVector(size, capacity,
                                                                                                        srcData,
                                                                                                        containNull) {
    ...
    public:
        FastMonthVector(int size, int capacity, int *srcData, bool containNull) : FastTemporalVector(size, capacity,
                                                                                                     srcData,
                                                                                                     containNull) {
            dataType_ = DT_MONTH;
        }
        ...
        virtual ConstantSP castTemporal(DATA_TYPE expectType) {
            if (expectType == DT_MONTH)
                ...
                throw RuntimeException(
                        "castTemporal from MONTH to " + Util::getDataTypeString(expectType) + " not supported ");
        }
    };

    class FastTimeVector : public FastTemporalVector {
    public:
        FastTimeVector(int size, int capacity, int *srcData, bool containNull) : FastTemporalVector(size, capacity,
                                                                                                    srcData,
                                                                                                    containNull) {
            dataType_ = DT_TIME;
        }
    ...
    public:
        FastMinuteVector(int size, int capacity, int *srcData, bool containNull) : FastTemporalVector(size, capacity,
                                                                                                      srcData,
                                                                                                      containNull) {
            dataType_ = DT_MINUTE;
        }
    ...
    public:
        FastSecondVector(int size, int capacity, int *srcData, bool containNull) : FastTemporalVector(size, capacity,
                                                                                                      srcData,
                                                                                                      containNull) {
            dataType_ = DT_SECOND;
        }
    ...
    public:
        FastNanoTimeVector(int size, int capacity, long long *srcData, bool containNull) : FastLongVector(size,
                                                                                                          capacity,
                                                                                                          srcData,
                                                                                                          containNull) {
    ...
    public:
        FastTimestampVector(int size, int capacity, long long *srcData, bool containNull) : FastLongVector(size,
                                                                                                           capacity,
                                                                                                           srcData,
                                                                                                           containNull) {
    ...
    public:
        FastNanoTimestampVector(int size, int capacity, long long *srcData, bool containNull) : FastLongVector(size,
                                                                                                               capacity,
                                                                                                               srcData,
                                                                                                               containNull) {
    ...
    public:
        FastBoolMatrix(int cols, int rows, int colCapacity, char *data, bool containNull) : Matrix(cols, rows),
                                                                                            FastBoolVector(cols * rows,
                                                                                                           colCapacity *
                                                                                                           rows, data,
                                                                                                           containNull) {
            setForm(DF_MATRIX);
        ...
        virtual bool setColumn(INDEX index, const ConstantSP &value) {
            fill(index * rows_, rows_, value);
            return true;
        }

        virtual ConstantSP getInstance(INDEX size) const {
            return Util::createMatrix(getType(), size, rows_, size, getExtraParamForType());
        }

        virtual ConstantSP getInstance() const {
            return Util::createMatrix(getType(), cols_, rows_, cols_, getExtraParamForType());
        }
    ...
    public:
        FastCharMatrix(int cols, int rows, int colCapacity, char *data, bool containNull) : Matrix(cols, rows),
                                                                                            FastCharVector(cols * rows,
                                                                                                           colCapacity *
                                                                                                           rows, data,
                                                                                                           containNull) {
            setForm(DF_MATRIX);
    ...
    public:
        FastShortMatrix(int cols, int rows, int colCapacity, short *data, bool containNull) : Matrix(cols, rows),
                                                                                              FastShortVector(
                                                                                                      cols * rows,
                                                                                                      colCapacity *
                                                                                                      rows, data,
    ...
    public:
        FastIntMatrix(int cols, int rows, int colCapacity, int *data, bool containNull) : Matrix(cols, rows),
                                                                                          FastIntVector(cols * rows,
                                                                                                        colCapacity *
                                                                                                        rows, data,
                                                                                                        containNull) {
            setForm(DF_MATRIX);
    ...
    public:
        FastLongMatrix(int cols, int rows, int colCapacity, long long *data, bool containNull) : Matrix(cols, rows),
                                                                                                 FastLongVector(
                                                                                                         cols * rows,
                                                                                                         colCapacity *
                                                                                                         rows, data,
    ...
    public:
        FastFloatMatrix(int cols, int rows, int colCapacity, float *data, bool containNull) : Matrix(cols, rows),
                                                                                              FastFloatVector(
                                                                                                      cols * rows,
                                                                                                      colCapacity *
                                                                                                      rows, data,
    ...
    public:
        FastDoubleMatrix(int cols, int rows, int colCapacity, double *data, bool containNull) : Matrix(cols, rows),
                                                                                                FastDoubleVector(
                                                                                                        cols * rows,
                                                                                                        colCapacity *
                                                                                                        rows, data,
        ...
        virtual string getString(int column, int row) const {
            return FastDoubleVector::getString(column * rows_ + row);
        }
    ...
    public:
        FastDateMatrix(int cols, int rows, int colCapacity, int *data, bool containNull) : Matrix(cols, rows),
                                                                                           FastDateVector(cols * rows,
                                                                                                          colCapacity *
                                                                                                          rows, data,
                                                                                                          containNull) {
            setForm(DF_MATRIX);
    ...
    public:
        FastDateTimeMatrix(int cols, int rows, int colCapacity, int *data, bool containNull) : Matrix(cols, rows),
                                                                                               FastDateTimeVector(
                                                                                                       cols * rows,
                                                                                                       colCapacity *
                                                                                                       rows, data,
    ...
    public:
        FastDateHourMatrix(int cols, int rows, int colCapacity, int *data, bool containNull) : Matrix(cols, rows),
                                                                                               FastDateHourVector(
                                                                                                       cols * rows,
                                                                                                       colCapacity *
                                                                                                       rows, data,
    ...
    public:
        FastMonthMatrix(int cols, int rows, int colCapacity, int *data, bool containNull) : Matrix(cols, rows),
                                                                                            FastMonthVector(cols * rows,
                                                                                                            colCapacity *
                                                                                                            rows, data,
                                                                                                            containNull) {
            setForm(DF_MATRIX);
    ...
    public:
        FastTimeMatrix(int cols, int rows, int colCapacity, int *data, bool containNull) : Matrix(cols, rows),
                                                                                           FastTimeVector(cols * rows,
                                                                                                          colCapacity *
                                                                                                          rows, data,
                                                                                                          containNull) {
            setForm(DF_MATRIX);
    ...
    public:
        FastSecondMatrix(int cols, int rows, int colCapacity, int *data, bool containNull) : Matrix(cols, rows),
                                                                                             FastSecondVector(
                                                                                                     cols * rows,
                                                                                                     colCapacity * rows,
                                                                                                     data,
    ...
    public:
        FastMinuteMatrix(int cols, int rows, int colCapacity, int *data, bool containNull) : Matrix(cols, rows),
                                                                                             FastMinuteVector(
                                                                                                     cols * rows,
                                                                                                     colCapacity * rows,
                                                                                                     data,
    ...
    public:
        FastNanoTimeMatrix(int cols, int rows, int colCapacity, long long *data, bool containNull) : Matrix(cols, rows),
                                                                                                     FastNanoTimeVector(
                                                                                                             cols *
                                                                                                             rows,
                                                                                                             colCapacity *
    ...
    public:
        FastTimestampMatrix(int cols, int rows, int colCapacity, long long *data, bool containNull) : Matrix(cols,
                                                                                                             rows),
                                                                                                      FastTimestampVector(
                                                                                                              cols *
                                                                                                              rows,
    ...
    public:
        FastNanoTimestampMatrix(int cols, int rows, int colCapacity, long long *data, bool containNull) : Matrix(cols,
                                                                                                                 rows),
                                                                                                          FastNanoTimestampVector(
                                                                                                                  cols *
                                                                                                                  rows,
                                                                                                                  colCapacity *
                                                                                                                  rows,
        ...
        virtual ConstantSP get(INDEX column, INDEX row) const {
            return FastNanoTimestampVector::get(column * rows_ + row);
        }
    };
    ...
    public:
        ...
        virtual int compare(INDEX index, const ConstantSP &target) const {
            return data_[index].compare(target->getString());
        }
        ...
        virtual ConstantSP getValue(INDEX capacity) const {
            return ConstantSP(new StringVector(data_, capacity, containNull_, blob_));
        }
    ...
    public:
        FastFixedLengthVector(DATA_TYPE type, int fixedLength, int size, int capacity, unsigned char *srcData,
                              bool containNull);
        ...
        virtual int
        serialize(char *buf, int bufSize, INDEX indexStart, int offset, int cellCountToSerialize, int &numElement,
                  int &partial) const;
    ...
    public:
        FastRecordVector(DATA_TYPE type, int size, int capacity, unsigned char *srcData, bool containNull)
                : FastFixedLengthVector(type, sizeof(T), size, capacity, srcData, containNull) {}
    ...
    public:
        ...
        virtual bool appendString(string *strarray, int len) {
            ...
            for (int i = 0; i < len; i++, strarray++, pdata += fixedLength_) {
                ...
                } else if (strarray->length() != 32 ||
                           Util::fromHex(strarray->data(), strarray->length(), Util::LITTLE_ENDIAN_ORDER, pdata) ==
                           false) {
                    return false;
                }
            }
    ...
    public:
        FastSymbolVector(SymbolBaseSP base, int size, int capacity, int *srcData, bool containNull)
                : AbstractFastVector(size, capacity, srcData, 0, containNull), base_(base) {
            if (size > 0) {
                base_->find("");
            }
        }
        ...
        virtual int compare(INDEX index, const ConstantSP &target) const {
            return base_->getSymbol(data_[index]).compare(target->getString());
        }
    ...
    namespace decimal_util {
        ...
        template<>
        struct MaxPrecision<int32_t> {
            static constexpr int value = 9;
        };
        template<>
        struct MaxPrecision<int64_t> {
        ...
        void assign(const Decimal<U> &other) {
            ...
            if (other.scale_ > scale_) {
                ...
                bool overflow = decimal_util::mulOverflow(static_cast<R>(other.rawData_),
                                                          decimal_util::scaleMultiplier<R>(scale_ - other.scale_),
                                                          value);
                if (overflow) {
                    throw RuntimeException("Decimal math overflow");
                }
	...
	*/
    ...
    public:/// Interface of Constant
        ...
        virtual void setDouble(double val) override {
            rawData_ = decimal_util::valueToDecimalraw<double, T>(val, scale_);
        }
	...
	*/
    ...
    public:
        ...
        virtual float getFloat(INDEX index) const {
            return decimal_util::decimalToValue<float, T>(data_[index], scale_);
        }
        ...
        virtual double getDouble(INDEX index) const {
            return decimal_util::decimalToValue<double, T>(data_[index], scale_);
        }
        ...
        virtual void setFloat(INDEX index, float val) {
            data_[index] = decimal_util::valueToDecimalraw<float, T>(val, scale_);
        }

        virtual void setDouble(INDEX index, double val) {
            data_[index] = decimal_util::valueToDecimalraw<double, T>(val, scale_);
        }
    ...
    public:
        ...
        virtual bool appendString(string *buf, int len) override {
            ...
            for (int i = 0; i < len; ++i) {
                if (false ==
                    decimal_util::parseString(buf[i].c_str(), buf[i].length(), data_[size_ + i], scale_, errMsg)) {
    ...
    public:
        ...
        virtual IO_ERR
        deserialize(DataInputStream *in, INDEX indexStart, INDEX targetNumElement, INDEX &numElement) override {
            IO_ERR ret = OK;
            INDEX end = indexStart + targetNumElement;
    ...
    public:
        ...
        getBinaryBuffer(INDEX start, int len, int unitLength, unsigned char *buf) const override {
            if (unitLength != static_cast<int>(sizeof(T))) {
                throw RuntimeException("Invalid unit length");
            }
            return reinterpret_cast<unsigned char *>(data_ + start);
