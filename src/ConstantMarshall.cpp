...
#define DLOG true ? DLogger::GetMinLevel() : DLogger::Info
    ...
    bool ScalarMarshall::start(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                               bool compress, IO_ERR &ret) {
        if (headerSize > 1024) {
            ret = INVALIDDATA;
            return false;
        ...
        int numElement = 0;
        int actualSize = 0;
        actualSize = target->serialize(buf_ + headerSize, MARSHALL_BUFFER_SIZE - headerSize, 0, 0, numElement,
                                       partial_);
        if (actualSize < 0) {
            ret = OTHERERR;
            return false;
    ...
    bool VectorMarshall::start(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                               bool compress, IO_ERR &ret) {
        if (headerSize > 1024) {
            ret = INVALIDDATA;
            return false;
    ...
    bool MatrixMarshall::sendMeta(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                                  IO_ERR &ret) {
        if (headerSize > 1024) {
            ret = INVALIDDATA;
            return false;
    ...
    bool MatrixMarshall::start(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                               bool compress, IO_ERR &ret) {
        target_.clear();
        rowLabelSent_ = false;
    ...
    bool TableMarshall::sendMeta(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                                 bool compress, IO_ERR &ret) {
        if (headerSize > 1024) {
            ret = INVALIDDATA;
            return false;
    ...
    bool TableMarshall::start(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                              bool compress, IO_ERR &ret) {
        target_.clear();
        columnNamesSent_ = 0;
        nextColumn_ = 0;
    ...
    bool SetMarshall::sendMeta(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                               IO_ERR &ret) {
        if (headerSize > 1024) {
            ret = INVALIDDATA;
            return false;
    ...
    bool SetMarshall::start(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                            bool compress, IO_ERR &ret) {
        target_.clear();
        complete_ = false;
    ...
    bool
    DictionaryMarshall::sendMeta(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                                 IO_ERR &ret) {
        if (headerSize > 1024) {
            ret = INVALIDDATA;
            return false;
    ...
    bool
    DictionaryMarshall::start(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                              bool compress, IO_ERR &ret) {
        target_.clear();
        keySent_ = false;
        inProgress_ = false;
    ...
    bool ChunkMarshall::start(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking,
                              bool compress, IO_ERR &ret) {
        if (headerSize > 1024) {
            ret = INVALIDDATA;
            return false;
    ...
    bool SymbolBaseMarshall::start(const SymbolBaseSP target, bool blocking, IO_ERR &ret) {
        ...
        nextStart_ = 0;
        partial_ = 0;
        actualSize = target->serialize(buf_ + sizeof(int) * 2, MARSHALL_BUFFER_SIZE - sizeof(int) * 2, 0, 0, numElement,
                                       partial_);
        if (actualSize < 0) {
            ret = OTHERERR;
            return false;
    ...
    bool VectorUnmarshall::start(short flag, bool blocking, IO_ERR &ret) {
        ...
        if (Util::getCategory(actualType) == DENARY || actualType == DT_DECIMAL32_ARRAY ||
            actualType == DT_DECIMAL64_ARRAY) {
    ...
    ConstantMarshallSP ConstantMarshallFactory::getInstance(DATA_FORM form, const DataOutputStreamSP &out) {
        switch (form) {
            case DF_SCALAR:
                return new ScalarMarshall(out);
            case DF_VECTOR:
            case DF_PAIR:
                return new VectorMarshall(out);
            case DF_MATRIX:
                return new MatrixMarshall(out);
            case DF_SET:
                return new SetMarshall(out);
            case DF_DICTIONARY:
                return new DictionaryMarshall(out);
            case DF_TABLE:
                return new TableMarshall(out);
            case DF_CHART:
                return new DictionaryMarshall(out);
            case DF_CHUNK:
                return new ChunkMarshall(out);
            default:
                return ConstantMarshallSP();
        }
    }
    ...
    ConstantUnmarshallSP ConstantUnmarshallFactory::getInstance(DATA_FORM form, const DataInputStreamSP &in) {
        switch (form) {
            case DF_SCALAR:
                return new ScalarUnmarshall(in);
            case DF_VECTOR:
            case DF_PAIR:
                return new VectorUnmarshall(in);
            case DF_MATRIX:
                return new MatrixUnmarshall(in);
            case DF_SET:
                return new SetUnmarshall(in);
            case DF_DICTIONARY:
                return new DictionaryUnmarshall(in);
            case DF_TABLE:
                return new TableUnmarshall(in);
            case DF_CHART:
                return new DictionaryUnmarshall(in);
            case DF_CHUNK:
                return new ChunkUnmarshall(in);
            default:
                return ConstantUnmarshallSP();
        }
    }
