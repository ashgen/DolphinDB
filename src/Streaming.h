...
#define DEFAULT_ACTION_NAME "cppStreamingAPI"
    ...
    public:
        ...
        StreamDeserializer(const unordered_map<string, pair<string, string>> &sym2tableName,
                           DBConnection *pconn = nullptr);
        ...
        bool
        parseBlob(const ConstantSP &src, vector<VectorSP> &rows, vector<string> &symbols, ErrorCodeInfo &errorInfo);
    ...
    protected:
        MessageQueueSP
        subscribeInternal(string host, int port, string tableName, string actionName = DEFAULT_ACTION_NAME,
    ...
    public:
        ...
        ThreadSP
        subscribe(const string &tableName, const IPCInMemoryTableReadHandler &handler, TableSP outputTable = nullptr,
                  bool overwrite = true);
