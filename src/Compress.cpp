...
namespace dolphindb {
    ...
    IO_ERR
    CompressionFactory::decode(DataInputStreamSP compressSrc, DataOutputStreamSP &uncompressResult, Header &header) {
    ...
    IO_ERR
    CompressionFactory::encodeContent(const VectorSP &vec, const DataOutputStreamSP &compressResult, Header &header,
                                      bool checkSum) {
        CompressEncoderDecoderSP decoder = GetEncodeDecoder((COMPRESS_METHOD) header.compressedType);
        if (decoder.isNull()) {
            return INVALIDDATA;
    ...
    int DeltaCompressor<T>::writeData(const T *data, int DataSize, long long *buf, int bufferSize) {
        ...
        while (count < DataSize) {
            if ((data[count] == INT_MIN && sizeof(T) == sizeof(int)) ||
                (data[count] == LLONG_MIN && sizeof(T) == sizeof(long long)) ||
                (data[count] == SHRT_MIN && sizeof(T) == sizeof(short))) {
    ...
    void DeltaCompressor<T>::writeFirstDelta(T data) {
        ...
        if (((previousData_ < 0 && blockData_ > 0 && previousDelta_ >= 0) ||
             (previousData_ > 0 && blockData_ < 0 && previousDelta_ <= 0)))
    ...
    void DeltaCompressor<T>::compressData(T data) {
        if ((data == INT_MIN && sizeof(T) == sizeof(int)) || (data == LLONG_MIN && sizeof(T) == sizeof(long long)) ||
            (data == SHRT_MIN && sizeof(T) == sizeof(short))) {
            compressDataNull();
            return;
        }
        ...
        if (((delta < 0 && previousDelta_ > 0 && deltaOfDelta >= 0) ||
             (delta > 0 && previousDelta_ < 0 && deltaOfDelta <= 0)))
            throw RuntimeException("Delta out of range");
        if (deltaOfDelta == 0) {
            write_.skipBit();
            previousData_ = (long long) data;
    ...
    static IO_ERR
    writeVectorMetaValue(const CompressionFactory::Header &header, BufferWriter<DataOutputStreamSP> &out) {
    ...
    IO_ERR CompressDeltaofDelta::decode(DataInputStreamSP compressSrc, DataOutputStreamSP &decompressResult,
                                        const CompressionFactory::Header &header) {
        ...
        while (fileCursor < byteSize && start < len) {
            ...
            if (blockSize <= 0 || blockSize > maxCompressedSize_) {
                std::cout << "Failed to decode. streamType=" + std::to_string(compressSrc->getStreamType()) +
                             " blockSize=" + std::to_string(blockSize) + " fileCursor=" +
                             std::to_string(fileCursor) + " fileLength=" + std::to_string(byteSize) + " decodedRows=" +
                             std::to_string(start) + " totalRows=" +
            ...
            if (ret != OK) {
                std::cout << "Failed to decode. fileCursor=" + std::to_string(fileCursor) + " fileLength=" +
                             std::to_string(byteSize) + " decodedRows=" +
                             std::to_string(start) + " totalRows=" + std::to_string(len) + "blockSize=" +
                             std::to_string(blockSize) + " actualRead=" +
            ...
            if (header.unitLength == 4) {
                ...
                actualRead = decoder.readData((long long *) compressedBuf, blockSize / sizeof(long long),
                                              (int *) decompressedBuf, count);
            } else if (header.unitLength == 8) {
                DeltaDecompressor<long long> decoder(LLONG_MIN);
                actualRead = decoder.readData((long long *) compressedBuf, blockSize / sizeof(long long),
                                              (long long *) decompressedBuf, count);
            } else {
                DeltaDecompressor<short> decoder(SHRT_MIN);
                actualRead = decoder.readData((long long *) compressedBuf, blockSize / sizeof(long long),
                                              (short *) decompressedBuf, count);
            }

            if (actualRead <= 0) {
                std::cout << "Failed to decode. LZ4 block offset=" + std::to_string(fileCursor - blockSize) +
                             " fileLength=" + std::to_string(byteSize) + " decodedRows=" + std::to_string(start) +
                             " totalRows=" + std::to_string(len) +
                             " blockSize=" + std::to_string(blockSize) + " bufSize=" +
                             std::to_string(decompressedBufSize)
                          << std::endl;
                return INVALIDDATA;
            }
            count = actualRead;
        ...
        INDEX start = 0;
        {
            ...
            while (start < len) {
                ...
                if (header.unitLength == 4) {
                    ...
                    blockSize = coder.writeData((const int *) p, count, compressedBuf,
                                                maxCompressedSize_ / sizeof(long long));
                } else if (header.unitLength == 8) {
                    const long long *p = vec->getLongConst(start, count, (long long *) decompressedBuf);
                    DeltaCompressor<long long> coder;
                    blockSize = coder.writeData((const long long *) p, count, compressedBuf,
                                                maxCompressedSize_ / sizeof(long long));
                } else {
                    const short *p = vec->getShortConst(start, count, (short *) decompressedBuf);
                    DeltaCompressor<short> coder;
                    blockSize = coder.writeData((const short *) p, count, compressedBuf,
                                                maxCompressedSize_ / sizeof(long long));
                }
    ...
    IO_ERR CompressLZ4::decode(DataInputStreamSP compressSrc, DataOutputStreamSP &uncompressResult,
                               const CompressionFactory::Header &header) {
        int unitLength = header.unitLength;
        long long fileCursor = 20;
        //long long lsn = -1;
        ...
        bool isMappingMode = (!compressSrc->isIntegerReversed() && type != DT_STRING && type != DT_BLOB &&
                              type < ARRAY_TYPE_BASE);
    ...
    IO_ERR CompressLZ4::encodeContent(const VectorSP &vec, const DataOutputStreamSP &compressResult,
                                      CompressionFactory::Header &header, bool needcheckSum) {
        std::vector<char *> blockBufList;
        std::vector<int> blockSizeList;
        int compressedbyteSize = 0;
        ...
        char *decompressedBuf = newBuffer(MAX_DECOMPRESSED_SIZE);//
        {
            ...
            if (type != DT_SYMBOL) {
                while (start < len) {
                    ...
                    if (type != DT_STRING) {
                        decompressedBufSize = vec->serialize(decompressedBuf, MAX_DECOMPRESSED_SIZE, start, offset,
                                                             count, offset);
                    } else {
                        decompressedBufSize = vec->serialize(decompressedBuf, MAX_DECOMPRESSED_SIZE, start, 0, count,
                                                             offset);
                    ...
                    blockSize = LZ4_compress_default(decompressedBuf, blockBuf + sizeof(int), decompressedBufSize,
                                                     MAX_COMPRESSED_SIZE);
