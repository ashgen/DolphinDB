...
namespace dolphindb {
    ...
    class ConstantFactory {
        ...
        typedef Vector *(ConstantFactory::*ConstantVectorFunc)(INDEX size, INDEX capacity, bool fastMode,
                                                               int extraParam, void *data, void **dataSegment,
                                                               int segmentSizeInBit, bool containNull);

        typedef Vector *(ConstantFactory::*ConstantArrayVectorFunc)(INDEX size, INDEX capacity, bool fastMode,
                                                                    int extraParam, void *data, void *pindex,
                                                                    void **dataSegment, int segmentSizeInBit,
                                                                    bool containNull);

        typedef Vector *(ConstantFactory::*ConstantMatrixFunc)(int cols, int rows, int colCapacity, int extraParam,
                                                               void *data, void **dataSegment, int segmentSizeInBit,
                                                               bool containNull);
    ...
    public:
        ConstantFactory()
                : arrConstParser{}, arrConstFactory{}, arrConstVectorFactory{}, arrConstArrayVectorFactory{},
                  arrConstMatrixFactory{}, arrTypeSymbol{} {
            init();
        }
        ...
        Vector *
        createConstantVector(DATA_TYPE type, INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data,
            ...
            if (func == NULL)
                ...
                return (this->*func)(size, capacity, fastMode, extraParam, data, dataSegment, segmentSizeInBit,
                                     containNull);
        }

        Vector *
        createConstantArrayVector(DATA_TYPE type, INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data,
                                  INDEX *pindex, void **dataSegment, int segmentSizeInBit, bool containNull) {
            ...
            if (func == NULL)
                ...
                return (this->*func)(size, capacity, fastMode, extraParam, data, (void *) pindex, dataSegment,
                                     segmentSizeInBit, containNull);
        }

        Vector *
        createConstantMatrix(DATA_TYPE type, int cols, int rows, int colCapacity, int extraParam, void *data = 0,
                             void **dataSegment = 0, int segmentSizeInBit = 0, bool containNull = false) {
            if (type < 0 || type >= TYPE_COUNT)
            ...
            if (func == NULL)
                ...
                return (this->*func)(cols, rows, colCapacity, extraParam, data, dataSegment, segmentSizeInBit,
                                     containNull);
        }
        ...
        Dictionary *createDictionary(DATA_TYPE keyInternalType, DATA_TYPE keyType, DATA_TYPE valueType) {
            if (valueType > DT_STRING &&
                (valueType != DT_UUID && valueType != DT_IP && valueType != DT_INT128 && valueType != DT_BLOB &&
                 valueType != DT_DATEHOUR && valueType != DT_DATEMINUTE)) {
                valueType = DT_ANY;
            }
            if (valueType != DT_ANY) {
                switch (keyInternalType) {
                    case DT_CHAR:
                        return new CharDictionary(keyType, valueType);
                    case DT_SHORT:
                        return new ShortDictionary(keyType, valueType);
                    case DT_INT:
                        return new IntDictionary(keyType, valueType);
                    case DT_LONG:
                        return new LongDictionary(keyType, valueType);
                    case DT_FLOAT:
                        return new FloatDictionary(valueType);
                    case DT_DOUBLE:
                        return new DoubleDictionary(valueType);
                    case DT_INT128:
                        if (valueType == DT_INT128)
                            return NULL;
                        return new Int128Dictionary(keyType, valueType);
                    case DT_STRING:
                        return new StringDictionary(keyType, valueType);
                    default:
                        return NULL;
                }
            } else {
                switch (keyInternalType) {
                    ...
                    case DT_SHORT:
                    case DT_INT:
                        return new IntAnyDictionary(keyType);
                    case DT_LONG:
                        return new LongAnyDictionary(keyType);
                    case DT_INT128:
                        return new Int128AnyDictionary(keyType);
                    case DT_STRING:
        ...
        Set *createSet(DATA_TYPE keyInternalType, DATA_TYPE keyType, INDEX capacity) {
            switch (keyInternalType) {
                case DT_CHAR:
                    return new CharSet(capacity);
                case DT_SHORT:
                    return new ShortSet(capacity);
                case DT_INT:
                    return new IntSet(keyType, capacity);
                case DT_LONG:
                    return new LongSet(keyType, capacity);
                case DT_FLOAT:
                    return new FloatSet(capacity);
                case DT_DOUBLE:
                    return new DoubleSet(capacity);
                case DT_STRING:
                    return new StringSet(capacity);
                case DT_INT128:
                    return new Int128Set(keyType, capacity);
                default:
                    return NULL;
            }
        }
    ...
    private:
        template<typename Y>
        void
        allocate(INDEX size, INDEX capacity, bool fastMode, int &segmentSizeInBit, void *&data, void **&dataSegment) {
            try {
        ...
        Vector *
        createVoidVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                         int segmentSizeInBit, bool containNull) {
            throw RuntimeException("Not allowed to create void vector");
        }

        Vector *
        createBoolVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                         int segmentSizeInBit, bool containNull) {
        ...
        Vector *
        createCharVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                         int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<char>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createShortVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                          int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
        ...
        Vector *
        createIntVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                        int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
        ...
        Vector *
        createLongVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                         int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
        ...
        Vector *
        createFloatVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                          int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
        ...
        Vector *
        createDoubleVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                           int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
        ...
        Vector *
        createDateVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                         int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createMonthVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                          int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createDateTimeVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                             int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createDateHourVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                             int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createTimeVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                         int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createNanoTimeVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                             int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createTimestampVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                              int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *createNanoTimestampVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data,
                                          void **dataSegment, int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createMinuteVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                           int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createSecondVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                           int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createStringVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                           int segmentSizeInBit, bool containNull) {
            return new StringVector(size, capacity);
        }

        Vector *
        createSymbolVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                           int segmentSizeInBit, bool containNull) {
            if (data) {
        ...
        Vector *
        createBlobVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                         int segmentSizeInBit, bool containNull) {
            return new StringVector(size, capacity, true);
        }

        Vector *
        createAnyVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                        int segmentSizeInBit, bool containNull) {
            return new AnyVector(size);
        }

        Vector *
        createInt128Vector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                           int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
        ...
        Vector *
        createUuidVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                         int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<Guid>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createIPAddrVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                           int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
                allocate<Guid>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            }
        ...
        Vector *
        createDecimal32Vector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                              int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
        ...
        Vector *
        createDecimal64Vector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void **dataSegment,
                              int segmentSizeInBit, bool containNull) {
            if (data == NULL && dataSegment == NULL) {
        ...
        Vector *
        createBoolArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                              void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<char>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull, DATA_TYPE(DT_BOOL + ARRAY_TYPE_BASE),
                                       (INDEX *) pindex);
        }

        Vector *
        createCharArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                              void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<char>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull, DATA_TYPE(DT_CHAR + ARRAY_TYPE_BASE),
                                       (INDEX *) pindex);
        }

        Vector *
        createShortArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                               void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<short>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_SHORT + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createIntArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                             void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull, DATA_TYPE(DT_INT + ARRAY_TYPE_BASE),
                                       (INDEX *) pindex);
        }

        Vector *
        createLongArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                              void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull, DATA_TYPE(DT_LONG + ARRAY_TYPE_BASE),
                                       (INDEX *) pindex);
        }

        Vector *
        createFloatArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                               void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<float>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_FLOAT + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createDoubleArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<double>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_DOUBLE + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createDateArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                              void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull, DATA_TYPE(DT_DATE + ARRAY_TYPE_BASE),
                                       (INDEX *) pindex);
        }

        Vector *
        createTimestampArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                   void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_TIMESTAMP + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createDateHourArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                  void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_DATEHOUR + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createDatetimeArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                  void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_DATETIME + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createTimeArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                              void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull, DATA_TYPE(DT_TIME + ARRAY_TYPE_BASE),
                                       (INDEX *) pindex);
        }

        Vector *
        createMinuteArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_MINUTE + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createMonthArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                               void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_MONTH + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createSecondArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<int>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_SECOND + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createNanotimeArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                  void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_NANOTIME + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *createNanoTimestampArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data,
                                               void *pindex, void **dataSegment, int segmentSizeInBit,
                                               bool containNull) {
            //if(data == NULL && dataSegment == NULL){
            //	allocate<long long>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_NANOTIMESTAMP + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *
        createUuidArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                              void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if (data == NULL && dataSegment == NULL) {
            //	allocate<Guid>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull, DATA_TYPE(DT_UUID + ARRAY_TYPE_BASE),
                                       (INDEX *) pindex);
        }

        Vector *
        createInt128ArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if (data == NULL && dataSegment == NULL) {
            //	allocate<Int128>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_INT128 + ARRAY_TYPE_BASE), (INDEX *) pindex);
        }

        Vector *createIpArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                    void **dataSegment, int segmentSizeInBit, bool containNull) {
            //if (data == NULL && dataSegment == NULL) {
            //	allocate<IPAddr>(size, capacity, fastMode, segmentSizeInBit, data, dataSegment);
            //}
            return new FastArrayVector(size, capacity, (char *) data, containNull, DATA_TYPE(DT_IP + ARRAY_TYPE_BASE),
                                       (INDEX *) pindex);
        }

        Vector *
        createDecimal32ArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                   void **dataSegment, int segmentSizeInBit, bool containNull) {
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_DECIMAL32 + ARRAY_TYPE_BASE), (INDEX *) pindex, extraParam);
        }

        Vector *
        createDecimal64ArrayVector(INDEX size, INDEX capacity, bool fastMode, int extraParam, void *data, void *pindex,
                                   void **dataSegment, int segmentSizeInBit, bool containNull) {
            return new FastArrayVector(size, capacity, (char *) data, containNull,
                                       DATA_TYPE(DT_DECIMAL64 + ARRAY_TYPE_BASE), (INDEX *) pindex, extraParam);
        }

        Vector *createVoidMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                 int segmentSizeInBit, bool containNull) {
            throw RuntimeException("Not allowed to create a void matrix");
        }

        Vector *createBoolMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                 int segmentSizeInBit, bool containNull) {
            if (data == 0)
        ...
        Vector *createCharMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                 int segmentSizeInBit, bool containNull) {
            if (data == 0)
        ...
        Vector *createShortMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                  int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastShortMatrix(cols, rows, colCapacity, allocateZero<short>(cols, rows, colCapacity),
                                           false);
            else
                return new FastShortMatrix(cols, rows, colCapacity, (short *) data, containNull);
        }

        Vector *createIntMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                int segmentSizeInBit, bool containNull) {
            if (data == 0)
        ...
        Vector *createLongMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                 int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastLongMatrix(cols, rows, colCapacity, allocateZero<long long>(cols, rows, colCapacity),
                                          false);
            else
                return new FastLongMatrix(cols, rows, colCapacity, (long long *) data, containNull);
        }

        Vector *createFloatMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                  int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastFloatMatrix(cols, rows, colCapacity, allocateZero<float>(cols, rows, colCapacity),
                                           false);
            else
                return new FastFloatMatrix(cols, rows, colCapacity, (float *) data, containNull);
        }

        Vector *createDoubleMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                   int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastDoubleMatrix(cols, rows, colCapacity, allocateZero<double>(cols, rows, colCapacity),
                                            false);
            else
                return new FastDoubleMatrix(cols, rows, colCapacity, (double *) data, containNull);
        }

        Vector *createDateMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                 int segmentSizeInBit, bool containNull) {
            if (data == 0)
        ...
        Vector *
        createDateTimeMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                             int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastDateTimeMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity),
                                              false);
            else
        ...
        Vector *
        createDateHourMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                             int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastDateHourMatrix(cols, rows, colCapacity, allocateZero<int>(cols, rows, colCapacity),
                                              false);
            else
                return new FastDateHourMatrix(cols, rows, colCapacity, (int *) data, containNull);
        }

        Vector *createMonthMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                  int segmentSizeInBit, bool containNull) {
            if (data == 0)
        ...
        Vector *createTimeMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                 int segmentSizeInBit, bool containNull) {
            if (data == 0)
        ...
        Vector *createSecondMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                   int segmentSizeInBit, bool containNull) {
            if (data == 0)
        ...
        Vector *createMinuteMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                   int segmentSizeInBit, bool containNull) {
            if (data == 0)
        ...
        Vector *
        createNanoTimeMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                             int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastNanoTimeMatrix(cols, rows, colCapacity, allocateZero<long long>(cols, rows, colCapacity),
                                              false);
            else
        ...
        Vector *
        createTimestampMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                              int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastTimestampMatrix(cols, rows, colCapacity,
                                               allocateZero<long long>(cols, rows, colCapacity), false);
            else
                return new FastTimestampMatrix(cols, rows, colCapacity, (long long *) data, containNull);
        }

        Vector *
        createNanoTimestampMatrix(int cols, int rows, int colCapacity, int extraParam, void *data, void **dataSegment,
                                  int segmentSizeInBit, bool containNull) {
            if (data == 0)
                return new FastNanoTimestampMatrix(cols, rows, colCapacity,
                                                   allocateZero<long long>(cols, rows, colCapacity), false);
            else
