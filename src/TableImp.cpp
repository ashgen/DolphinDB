...
namespace dolphindb {
    ...
    AbstractTable::AbstractTable(const SmartPointer<vector<string>> &colNames,
                                 SmartPointer<unordered_map<string, int>> colMap) : colNames_(colNames),
                                                                                    colMap_(colMap) {
    }
    ...
    ConstantSP
    AbstractTable::getColumn(const string &qualifier, const string &name, const ConstantSP &rowFilter) const {
        if (rowFilter.isNull())
    ...
    void AbstractTable::setColumnCompressMethods(const vector<COMPRESS_METHOD> &colCompresses) {
        if (colCompresses.size() > 0 && colCompresses.size() != colNames_->size()) {
            throw RuntimeException(
                    "The number of elements in parameter compressMethods does not match the column size " +
                    std::to_string(colNames_->size()) + ".");
        }
        for (size_t i = 0; i < colCompresses.size(); i++) {
            if (colCompresses[i] == COMPRESS_DELTA) {
                ...
                if (dataType != DT_SHORT && dataType != DT_INT && dataType != DT_LONG) {
                    throw RuntimeException("Cannot apply compression method DELTA to column " + colNames_->at(i) +
                                           ", Only SHORT/INT/LONG or temporal data supports DELTA compression");
                }
                if (((Vector *) getColumn(i).get())->getVectorType() == VECTOR_TYPE::ARRAYVECTOR) {
                    throw RuntimeException(
                            "Cannot apply compression method DELTA to array vector at column " + colNames_->at(i));
                }
    ...
    ConstantSP AbstractTable::getWindowInternal(int columnStart, int columnLength, int rowStart, int rowLength) const {
        ...
        if (rowStart == 0 && rowLength == size()) {
            ...
            for (int i = 0; i < columnLength; i++)
                newCols.push_back(
                        ((Vector *) getColumn(columnStart + sign * i).get())->getSubVector(rowStart, rowLength));
        }
    ...
    bool AbstractTable::update(vector<ConstantSP> &values, const ConstantSP &indexSP, vector<string> &colNames,
                               string &errMsg) {
    ...
    BasicTable::BasicTable(const vector<ConstantSP> &cols, const vector<string> &colNames) : AbstractTable(
            new vector<string>(colNames)) {
        initData(cols, colNames);
    }

    BasicTable::BasicTable(const vector<ConstantSP> &cols, const vector<string> &colNames, const vector<int> &keys)
            : AbstractTable(new vector<string>(colNames)) {
        initData(cols, colNames);
    }
...
*/
    bool BasicTable::update(vector<ConstantSP> &values, const ConstantSP &indexSP, vector<string> &colNames,
                            string &errMsg) {
        if (isReadOnly()) {
            errMsg = "Can't modify read only table.";
            return false;
    ...
    bool BasicTable::internalUpdate(vector<ConstantSP> &values, const ConstantSP &indexSP, vector<string> &colNames,
                                    string &errMsg) {
        int num = values.size();
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (it == colMap_->end()) {
                ...
                if (!allowNewCol || (!values[i]->isScalar() && values[i]->size() != rows)) {
                    errMsg.append("The column " + colNames[i] +
                                  " does not exist. To add a new column, the table shouldn't be shared and the value size must match the table.");
                    return false;
                } else {
                ...
                if (cols_[colIndex[i]]->getCategory() != values[i]->getCategory() &&
                    (!values[i]->isNumber() || !cols_[colIndex[i]]->isNumber()) &&
                    values[i]->getCategory() != NOTHING) {
    ...
    bool BasicTable::join(vector<ConstantSP> &columns) {
        ...
        for (int i = 0; i < num; ++i) {
            ...
            if (!col->isArray() || col->size() != size_ || name.empty() ||
                colMap_->find(Util::lower(name)) != colMap_->end())
                return false;
        }
        for (int i = 0; i < num; ++i) {
            ConstantSP &col = columns[i];
