...
namespace dolphindb {
    ...
    public:
        MatrixMarshall(const DataOutputStreamSP &out) : ConstantMarshallImp(out), rowLabelSent_(false),
                                                        columnLabelSent_(false), inProgress_(false),
                                                        vectorMarshall_(out) {}
    ...
    private:
        bool
        sendMeta(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking, IO_ERR &ret);
    ...
    private:
        bool
        sendMeta(const char *requestHeader, size_t headerSize, const ConstantSP &target, bool blocking, bool compress,
                 IO_ERR &ret);
    ...
    public:
        ScalarUnmarshall(const DataInputStreamSP &in) : ConstantUnmarshallImp(in), isCodeObject_(false), scale_(0),
                                                        functionType_(-1) {}
    ...
    public:
        VectorUnmarshall(const DataInputStreamSP &in) : ConstantUnmarshallImp(in), flag_(0), rows_(0), columns_(0),
                                                        nextStart_(0), unmarshall_(0), scale_(0) {}
    ...
    public:
        MatrixUnmarshall(const DataInputStreamSP &in) : ConstantUnmarshallImp(in), labelFlag_(-1),
                                                        rowLabelReceived_(false),
                                                        columnLabelReceived_(false), inProgress_(false),
                                                        vectorUnmarshall_(in) {}
    ...
    public:
        TableUnmarshall(const DataInputStreamSP &in) : ConstantUnmarshallImp(in), type_(BASICTBL),
                                                       tableNameReceived_(false), columnNameReceived_(false),
                                                       nextColumn_(0), inProgress_(false), rows_(0), columns_(0),
                                                       vectorUnmarshall_(in) {}
        ...
        void setNotify(std::function<void(const ConstantSP &vector, INDEX validSize)> notify) {
            vectorUnmarshall_.setNotify(notify);
        }
    ...
    public:
        SetUnmarshall(const DataInputStreamSP &in) : ConstantUnmarshallImp(in), inProgress_(false),
                                                     vectorUnmarshall_(in) {}
    ...
    public:
        ...
        ConstantMarshall *getConstantMarshall(DATA_FORM form) {
            return (form < 0 || form > DF_CHUNK) ? NULL : arrMarshall[form];
        }
    ...
    public:
        ...
        ConstantUnmarshall *getConstantUnmarshall(DATA_FORM form) {
            return (form < 0 || form > DF_CHUNK) ? NULL : arrUnmarshall[form];
        }
