...
namespace dolphindb {
    ...
    BatchTableWriter::BatchTableWriter(const std::string &hostName, int port, const std::string &userId,
                                       const std::string &password, bool acquireLock) : hostName_(hostName),
                                                                                        port_(port),
                                                                                        userId_(userId),
                                                                                        password_(password),
    ...
    void BatchTableWriter::addTable(const string &dbName, const string &tableName, bool partitioned) {
        ...
        if (tableName.empty()) {
            ...
            saveTable = std::move(
                    std::string("saveTable(database(\"") + dbName + "\")" + "," + tmpDiskGlobal + ",\"" + tableName +
                    "\", 1)");
            schema = conn.run(std::string("schema(loadTable(\"") + dbName + "\",\"" + tableName + "\"))");
        }
        ...
        if (tmpDiskGlobal.empty() == false) {//update need temp table
            ...
            destTable->createTmpSharedTable = std::move(
                    std::string("share table(") + "1000:0," + colNames + "," + colTypes + ") as " + tmpDiskGlobal);
        }
    ...
    bool BatchTableWriter::writeTableAllData(SmartPointer<DestTable> destTable, bool partitioned) {
        ...
        {
            destTableRawPtr->writeTable = Util::createTable(destTableRawPtr->colNames, destTableRawPtr->colTypes, 0,
                                                            size);
            INDEX insertedRows;
            std::string errMsg;
            for (int i = 0; i < size; i++) {
                ...
                if (!ret) {
                    std::cerr << Util::createTimestamp(Util::getEpochTime())->getString()
                              << " Backgroud thread of table (" << destTableRawPtr->dbName << " "
                              << destTableRawPtr->tableName << "). Failed to create table, with error: " << errMsg
                              << std::endl;
                    writedataok = false;
                    break;
                }
        ...
        if (writedataok) {
            try {
                ...
                writedataok = false;
                std::cerr << Util::createTimestamp(Util::getEpochTime())->getString() << " Backgroud thread of table ("
                          << destTableRawPtr->dbName << " " << destTableRawPtr->tableName
                          << "). Failed to send data to server, with exception: " << e.what() << std::endl;
            }
        }
    ...
    TableSP BatchTableWriter::getAllStatus() {
        int columnNum = 6;
        std::vector<std::string> colNames{"DatabaseName", "TableName", "WriteQueueDepth", "SendedRows", "Removing",
                                          "Finished"};
        std::vector<DATA_TYPE> colTypes{DATA_TYPE::DT_STRING, DATA_TYPE::DT_STRING, DATA_TYPE::DT_INT,
                                        DATA_TYPE::DT_INT, DATA_TYPE::DT_BOOL, DATA_TYPE::DT_BOOL};
    ...
    TableSP BatchTableWriter::getUnwrittenData(const string &dbName, const string &tableName) {
        ...
        if (destTables_.find(std::make_pair(dbName, tableName)) == destTables_.end())
            throw RuntimeException(
                    "Failed to get unwritten data. Please use addTable to add infomation of database and table first.");
        SmartPointer<DestTable> destTable = destTables_[std::make_pair(dbName, tableName)];
