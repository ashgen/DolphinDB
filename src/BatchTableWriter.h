...
namespace dolphindb {
     ...
     */
        BatchTableWriter(const std::string &hostName, int port, const std::string &userId, const std::string &password,
     ...
     */
        ...
        void insert(const string &dbName, const string &tableName, Targs... Fargs) {
            ...
            SmartPointer<DestTable> destTable;
            {
                ...
                if (destTables_.find(std::make_pair(dbName, tableName)) == destTables_.end())
                    throw RuntimeException(
                            "Failed to insert into table, please use addTable to add infomation of database and table first.");
            ...
            if (argSize != destTable->columnNum)
                throw RuntimeException(
                        "Failed to insert into table, number of arguments must match the number of columns of table.");
            if (argSize == 0)
                return;
    ...
    private:
        ...
        void insertRecursive(std::vector<ConstantSP> *row, DestTable *destTable, int colIndex) {
            ...
            if (destTable->finished) {
                throw RuntimeException(std::string(
                        "Failed to insert data. Error writing data in backgroud thread. Please use getUnwrittenData to get data not written to server and remove talbe (") +
                                       destTable->dbName + " " + destTable->tableName + ").");
            }
        ...
        void
        insertRecursive(std::vector<ConstantSP> *row, DestTable *destTable, int colIndex, T first, Targs... Fargs) {
            ConstantSP tmp;
            try {
                tmp = createObject(destTable->colDefsTypeInt->getInt(colIndex), first);
            } catch (RuntimeException &e) {
                throw RuntimeException(
                        "Failed to insert data, unsupported data type at column: " + std::to_string(colIndex));
            }
            if (destTable->colDefsTypeInt->getInt(colIndex) != DT_SYMBOL &&
                int(tmp->getType()) != destTable->colDefsTypeInt->getInt(colIndex))
                throw RuntimeException(
                        "Failed to insert data, the type of argument does not match the type of column at column: " +
