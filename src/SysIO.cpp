...
#define RECORD_WRITE(pbytes, bytelen)//Util::writeFile("/tmp/ddb_write.bin", pbytes, bytelen);
    ...
    Socket::Socket() : host_(""), port_(-1), blocking_(true), autoClose_(true), enableSSL_(false), ctx_(nullptr),
                       ssl_(nullptr), keepAliveTime_(30) {
    ...
    Socket::Socket(const string &host, int port, bool blocking, int keepAliveTime, bool enableSSL) : host_(host),
                                                                                                     port_(port),
                                                                                                     blocking_(
                                                                                                             blocking),
                                                                                                     autoClose_(true),
    ...
    Socket::Socket(SOCKET handle, bool blocking, int keepAliveTime) : host_(""), port_(-1), handle_(handle),
                                                                      blocking_(blocking), autoClose_(true),
                                                                      enableSSL_(false), ctx_(nullptr), ssl_(nullptr),
...
#else //Linux
            readdata:
            actualLength = recv(handle_, (void *) buffer, length,
                                (blocking_ ? 0 : MSG_DONTWAIT) | (msgPeek ? MSG_PEEK : 0));
            RECORD_READ(buffer, actualLength);
            if (actualLength < 0) {
                DLogger::Error("socket read error", actualLength);
            }
...
#else
            senddata:
            actualLength = send(handle_, (const void *) buffer, length,
                                blocking_ ? MSG_NOSIGNAL : MSG_DONTWAIT | MSG_NOSIGNAL);
            RECORD_WRITE(buffer, actualLength);
    ...
    IO_ERR Socket::bind() {
        ...
        if (SOCKET_ERROR == r) {
            LOG_ERR("Failed to bind the socket on port " + std::to_string(port_) + " with error code " +
                    std::to_string(getErrorCode()));
            closesocket(handle_);
            return OTHERERR;
        }
        return OK;
    ...
    IO_ERR Socket::connect() {
        ...
        if (getaddrinfo(host_.c_str(), portStr.c_str(), &hints, &servinfo) != 0) {
            LOG_ERR("Failed to call getaddrinfo for host = " + host_ + " port = " + portStr + " with error code " +
                    std::to_string(getErrorCode()));
            return OTHERERR;
        }
        ...
        for (p = servinfo; p != NULL; p = p->ai_next) {
            ...
            if (::connect(handle_, p->ai_addr, p->ai_addrlen) == SOCKET_ERROR) {
                ...
                LOG_ERR("Failed to connect to host = " + host_ + " port = " + portStr + " with error code " +
                        std::to_string(getErrorCode()));
                closesocket(handle_);
                handle_ = INVALID_SOCKET;
                continue;
    ...
    UdpSocket::UdpSocket(const string &remoteHost, int remotePort) : port_(0), remoteHost_(remoteHost),
    ...
    IO_ERR UdpSocket::bind() {
        ...
        if (SOCKET_ERROR == r) {
            LOG_ERR("Failed to bind the udp socket on port " + std::to_string(port_) + " with error code " +
                    std::to_string(getErrorCode()));
            closesocket(handle_);
            return OTHERERR;
        }
    ...
    DataInputStream::DataInputStream(STREAM_TYPE type, int bufSize) : file_(0), buf_(new char[bufSize]), source_(type),
                                                                      reverseOrder_(false), externalBuf_(false),
                                                                      closed_(false), capacity_(bufSize), size_(0),
                                                                      cursor_(0) {
    }

    DataInputStream::DataInputStream(const SocketSP &socket, int bufSize) : socket_(socket), file_(0),
                                                                            buf_(new char[bufSize]),
                                                                            source_(SOCKET_STREAM),
                                                                            reverseOrder_(false),
                                                                            externalBuf_(false), closed_(false),
                                                                            capacity_(bufSize), size_(0), cursor_(0) {
    }

    DataInputStream::DataInputStream(FILE *file, int bufSize) : file_(file), buf_(new char[bufSize]),
                                                                source_(FILE_STREAM), reverseOrder_(false),
                                                                externalBuf_(false),
                                                                closed_(false), capacity_(bufSize), size_(0),
                                                                cursor_(0) {
    }

    DataInputStream::DataInputStream(const char *data, int size, bool copy) : file_(0), source_(ARRAY_STREAM),
                                                                              reverseOrder_(false), externalBuf_(!copy),
                                                                              closed_(false),
    ...
    DataInputStream::DataInputStream(DataQueueSP dataQueue) : file_(nullptr), buf_(nullptr), source_(QUEUE_STREAM),
                                                              reverseOrder_(false), externalBuf_(false),
                                                              closed_(false), capacity_(0), size_(0), cursor_(0),
    ...
    DataOutputStream::DataOutputStream(const SocketSP &socket, size_t flushThreshold) : source_(SOCKET_STREAM),
                                                                                        ...
                                                                                        socket_(socket), file_(0),
                                                                                        buf_(0),
                                                                                        capacity_(flushThreshold * 2),
                                                                                        size_(0), autoClose_(false) {
        if (capacity_ > 0)
            buf_ = new char[capacity_];
    }

    DataOutputStream::DataOutputStream(FILE *file, bool autoClose) : source_(FILE_STREAM), flushThreshold_(0),
                                                                     file_(file),
                                                                     buf_(0), capacity_(0), size_(0),
                                                                     autoClose_(autoClose) {
    }

    DataOutputStream::DataOutputStream(size_t capacity) : source_(ARRAY_STREAM), flushThreshold_(0), file_(0),
                                                          buf_(new char[capacity]),
                                                          capacity_(capacity), size_(0), autoClose_(false) {
    }

    DataOutputStream::DataOutputStream(STREAM_TYPE source) : source_(source), flushThreshold_(0), file_(0), buf_(0),
                                                             capacity_(0), size_(0), autoClose_(false) {
    }

    DataOutputStream::DataOutputStream(DataQueueSP dataQueue) : source_(QUEUE_STREAM), flushThreshold_(0), file_(0),
                                                                buf_(0), capacity_(0), size_(0), autoClose_(false),
                                                                dataQueue_(dataQueue) {
    }
    ...
    IO_ERR DataOutputStream::close() {
        if (source_ == SOCKET_STREAM) {
            ...
            if (size_ > 0) {
                ...
                if (ret != OK)
                    LOG_ERR("Failed to write cached data to the underlying device before closing. size=" +
                            std::to_string(size_) + " streamType=" + std::to_string(source_));
                size_ = 0;
            }
    ...
    IO_ERR DataOutputStream::resume() {
        ...
        while (size_ > 0 && (ret = socket_->write(buf_ + cursor, size_, actualWritten)) == OK &&
               actualWritten <= size_) {
