...
public:
    DBConnectionImpl(bool sslEnable = false, bool asynTask = false, int keepAliveTime = 30);
    ~DBConnectionImpl();
    bool connect(const string& hostName, int port, const string& userId = "", const string& password = "",bool sslEnable = false, bool asynTask = false, int keepAliveTime = -1);
...
DBConnectionImpl::DBConnectionImpl(bool sslEnable, bool asynTask, int keepAliveTime): port_(0), encrypted_(false), isConnected_(false), littleEndian_(Util::isLittleEndian()), sslEnable_(sslEnable),asynTask_(asynTask), keepAliveTime_(keepAliveTime){
    if (!initialized_)
        initialize();
}
...
bool DBConnectionImpl::connect(const string& hostName, int port, const string& userId, const string& password, bool sslEnable,bool asynTask, int keepAliveTime) {
    hostName_ = hostName;
    port_ = port;
    userId_ = userId;
    pwd_ = password;
...
DBConnection::DBConnection(bool enableSSL, bool asynTask, int keepAliveTime) : conn_(new DBConnectionImpl(enableSSL, asynTask, keepAliveTime)), uid_(""), pwd_(""), ha_(false), enableSSL_(enableSSL),asynTask_(asynTask), nodes_(nullptr) {
...
bool DBConnection::connect(const string& hostName, int port, const string& userId, const string& password, const string& startup,
    ...
    if (ha_) {
        while (true) {
            if (conn_->connect(hostName, port, userId, password, enableSSL_, asynTask_, keepAliveTime)) {
                uid_ = userId;
                pwd_ = password;
                break;
            }
        ...
        bool ok = conn_->connect(hostName, port, userId, password, enableSSL_, asynTask_, keepAliveTime);
