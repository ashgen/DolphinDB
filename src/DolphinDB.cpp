...
namespace dolphindb {
    ...
    public:
        DBConnectionImpl(bool sslEnable = false, bool asynTask = false, int keepAliveTime = 7200, bool compress = false,
                         bool python = false, bool isReverseStreaming = false);

        ~DBConnectionImpl();

        bool connect(const string &hostName, int port, const string &userId = "", const string &password = "",
                     bool sslEnable = false, bool asynTask = false, int keepAliveTime = -1, bool compress = false,
                     bool python = false);

        void login(const string &userId, const string &password, bool enableEncryption);

        ConstantSP
        run(const string &script, int priority = 4, int parallelism = 2, int fetchSize = 0, bool clearMemory = false);

        ConstantSP
        run(const string &funcName, vector<ConstantSP> &args, int priority = 4, int parallelism = 2, int fetchSize = 0,
    ...
    private:
        long generateRequestFlag(bool clearSessionMemory = false, bool disablepickle = false,
                                 bool pickleTableToList = false);

        ConstantSP run(const string &script, const string &scriptType, vector<ConstantSP> &args, int priority = 4,
                       int parallelism = 2, int fetchSize = 0, bool clearMemory = false);
    ...
    public:
        ...
        struct Result {
            Result(TASK_STAGE s = WAITING, const ConstantSP c = Constant::void_, const string &msg = "") : stage(s),
                                                                                                           result(c),
                                                                                                           errMsg(msg) {}
    ...
    public:
        struct Task {
            ...
            Task(const string &function, const vector<ConstantSP> &args, int id = 0, int pr = 4, int pa = 2,
                 bool clearM = false)
                    : script(function), arguments(args), identity(id), priority(pr), parallelism(pa),
                      clearMemory(clearM) { isFunc = true; }
        ...
        DBConnectionPoolImpl(const string &hostName, int port, int threadNum = 10, const string &userId = "",
                             ...
                             bool loadBalance = true, bool highAvailability = true, bool compress = false,
                             bool reConnect = false, bool python = false);
        ...
        void run(const string &script, int identity, int priority = 4, int parallelism = 2, int fetchSize = 0,
                 bool clearMemory = false) {
            queue_->push(Task(script, identity, priority, parallelism, clearMemory));
            taskStatus_.setResult(identity, TaskStatusMgmt::Result());
        }

        void run(const string &functionName, const vector<ConstantSP> &args, int identity, int priority = 4,
                 int parallelism = 2, int fetchSize = 0, bool clearMemory = false) {
    ...
    int
    Constant::serialize(char *buf, int bufSize, INDEX indexStart, int offset, int cellCountToSerialize, int &numElement,
                        int &partial) const {
        throw RuntimeException(Util::getDataFormString(getForm()) + "_" + Util::getDataTypeString(getType()) +
                               " serialize cell method not supported");
    }
    ...
    IO_ERR Constant::deserialize(DataInputStream *in, INDEX indexStart, INDEX targetNumElement, INDEX &numElement) {
        throw RuntimeException(Util::getDataFormString(getForm()) + "_" + Util::getDataTypeString(getType()) +
                               " deserialize method not supported");
    }
    ...
    Matrix::Matrix(int cols, int rows) : cols_(cols), rows_(rows), rowLabel_(Constant::void_),
    ...
    ConstantSP Matrix::get(const ConstantSP &index) const {
        if (index->isScalar()) {
            ...
            if (col < 0 || col >= cols_)
                throw OperatorRuntimeException("matrix",
                                               "The column index " + Util::convert(col) + " is out of range.");
        ...
        for (int i = 0; i < cols; ++i) {
            ...
            if (cur < 0 || cur >= cols_)
                throw OperatorRuntimeException("matrix",
                                               "The column index " + Util::convert(cur) + " is out of range.");
    ...
    DFSChunkMeta::DFSChunkMeta(const string &path, const Guid &id, int version, int size, CHUNK_TYPE chunkType,
                               const vector<string> &sites, long long cid)
            : Constant(2051), type_(chunkType), replicaCount_(sites.size()), version_(version), size_(size), sites_(0),
              path_(path), cid_(cid), id_(id) {
        if (replicaCount_ == 0)
            return;
    ...
    DFSChunkMeta::DFSChunkMeta(const string &path, const Guid &id, int version, int size, CHUNK_TYPE chunkType,
                               const string *sites, int siteCount, long long cid)
            : Constant(2051), type_(chunkType), replicaCount_(siteCount), version_(version), size_(size), sites_(0),
              path_(path), cid_(cid), id_(id) {
        if (replicaCount_ == 0)
            return;
        sites_ = new string[replicaCount_];
    ...
    bool DBConnectionImpl::connect(const string &hostName, int port, const string &userId,
                                   const string &password, bool sslEnable, bool asynTask, int keepAliveTime,
                                   bool compress,
                                   bool python) {
        hostName_ = hostName;
        port_ = port;
    ...
    bool DBConnectionImpl::connect() {
        ...
        if (ret != OK)
            throw IOException(
                    "Couldn't send login message to the given host/port with IO error type " + std::to_string(ret));
        ...
        if ((ret = in->readLine(line)) != OK)
            throw IOException(
                    "Failed to read response message from the socket with IO error type " + std::to_string(ret));
        if (line != "OK")
        ...
        if (numObject == 1) {
            ...
            if ((ret = in->readShort(flag)) != OK)
                throw IOException(
                        "Failed to read object flag from the socket with IO error type " + std::to_string(ret));
        ...
        if (!requiredVersion->isTuple()) {
            ...
            if (apiVersion > APIMinVersionRequirement) {
                close();
                throw IOException("Required C++ API version at least " + std::to_string(apiVersion) +
                                  ". Current C++ API version is " + std::to_string(APIMinVersionRequirement) +
                                  ". Please update DolphinDB C++ API. ");
            }
    ...
    ConstantSP
    DBConnectionImpl::run(const string &script, int priority, int parallelism, int fetchSize, bool clearMemory) {
        vector<ConstantSP> args;
        return run(script, "script", args, priority, parallelism, fetchSize, clearMemory);
    }

    ConstantSP DBConnectionImpl::run(const string &funcName, vector<ConstantSP> &args, int priority, int parallelism,
    ...
    ConstantSP DBConnectionImpl::run(const string &script, const string &scriptType, vector<ConstantSP> &args,
        ...
        if (!isConnected_)
            throw IOException(
                    "Couldn't send script/function to the remote host because the connection has been closed");
        ...
        out.append(Util::convert((int) body.size()));
        out.append(" / " + std::to_string(generateRequestFlag(clearMemory, true)) + "_1_" + std::to_string(priority) +
                   "_" + std::to_string(parallelism));
        if (fetchSize > 0)
        ...
        if (argCount > 0) {
            ...
            for (int i = 0; i < argCount; ++i) {
                ...
                if (ret != OK) {
                    close();
                    throw IOException("Couldn't send function argument to the remote host with IO error type " +
                                      std::to_string(ret));
                }
            }
            ret = outStream->flush();
        ...
        if ((ret = in->readLine(line)) != OK) {
            close();
            throw IOException(
                    "Failed to read response header from the socket with IO error type " + std::to_string(ret));
        }
        while (line == "MSG") {
            if ((ret = in->readString(line)) != OK) {
                close();
                throw IOException(
                        "Failed to read response msg from the socket with IO error type " + std::to_string(ret));
            }
        ...
        if (line != "OK") {
            throw IOException(
                    hostName_ + ":" + std::to_string(port_) + " Server response: '" + line + "' script: '" + script +
                    "'");
        }
    ...
    DBConnection::DBConnection(bool enableSSL, bool asyncTask, int keepAliveTime, bool compress, bool python,
                               bool isReverseStreaming) : conn_(
            new DBConnectionImpl(enableSSL, asyncTask, keepAliveTime, compress, python, isReverseStreaming)), uid_(""),
                                                          pwd_(""), ha_(false),
                                                          enableSSL_(enableSSL), asynTask_(asyncTask),
                                                          compress_(compress), nodes_({}),
                                                          lastConnNodeIndex_(0), python_(python), reconnect_(false),
                                                          closed_(true) {
    }

    DBConnection::DBConnection(DBConnection &&oth) : conn_(move(oth.conn_)), uid_(move(oth.uid_)), pwd_(move(oth.pwd_)),
                                                     initialScript_(move(oth.initialScript_)), ha_(oth.ha_),
                                                     enableSSL_(oth.enableSSL_),
                                                     asynTask_(oth.asynTask_), compress_(oth.compress_),
                                                     nodes_(oth.nodes_), lastConnNodeIndex_(0),
            ...
            for (int i = 0; i < colMode->rows(); i++) {
                if (colMode->getInt(i) == 0) {
                    ...
                    if (colconnectionNum->getInt(i) < colmaxConnections->getInt(i)) {
                        load = (colconnectionNum->getInt(i) + colworkerNum->getInt(i) + colexecutorNum->getInt(i)) /
                               3.0;
    ...
    bool DBConnection::connectNode(string hostName, int port, int keepAliveTime) {
        ...
        while (closed_ == false) {
            try {
                return conn_->connect(hostName, port, uid_, pwd_, enableSSL_, asynTask_, keepAliveTime, compress_,
                                      python_);
            } catch (IOException &e) {
                if (connected()) {
                    ...
                    if (type != ET_NEWLEADER) {
                        if (type == ET_IGNORE)
                            ...
                            std::cerr << "Connect " << hostName << ":" << port << " failed, exception message: "
                                      << e.what() << std::endl;
                            return false;
                            //throw;
                        }
    ...
    ConstantSP
    DBConnection::run(const string &funcName, vector<dolphindb::ConstantSP> &args, int priority, int parallelism,
                      int fetchSize, bool clearMemory) {
        if (nodes_.empty() == false) {
            while (closed_ == false) {
                try {
    ...
    Domain::Domain(PARTITION_TYPE partitionType, DATA_TYPE partitionColType) : partitionType_(partitionType),
                                                                               partitionColType_(partitionColType) {
    ...
    DBConnectionPoolImpl::DBConnectionPoolImpl(const string &hostName, int port, int threadNum, const string &userId,
                                               const string &password,
                                               bool loadBalance, bool highAvailability, bool compress, bool reConnect,
                                               bool python) : shutDownFlag_(false),
                                                              queue_(new SynchronizedQueue<Task>) {
        ...
        if (!loadBalance) {
            for (int i = 0; i < threadNum; i++) {
                ...
                workers_.push_back(new Thread(
                        new AsynWorker(*this, latch_, conn, queue_, taskStatus_, hostName, port, userId, password)));
                workers_.back()->start();
            }
        } else {
            ...
            for (int i = 0; i < threadNum; i++) {
                ...
                workers_.push_back(new Thread(
                        new AsynWorker(*this, latch_, conn, queue_, taskStatus_, curhost, curport, userId, password)));
                workers_.back()->start();
            }
        }
    }
    ...
    void AsynWorker::run() {
        while (true) {
            ...
            while (true) {
                try {
                    if (task.isFunc) {
                        result = conn_->run(task.script, task.arguments, task.priority, task.parallelism, 0,
                                            task.clearMemory);
                    } else {
                    ...
                    taskStatus_.setResult(task.identity,
                                          TaskStatusMgmt::Result(TaskStatusMgmt::ERRORED, new Void(), ex.what()));
                    break;
    ...
    bool TaskStatusMgmt::isFinished(int identity) {
        ...
        if (results[identity].stage == ERRORED)
            throw RuntimeException(
                    "Task [" + std::to_string(identity) + "] come across exception : " + results[identity].errMsg);
    ...
    ConstantSP TaskStatusMgmt::getData(int identity) {
        ...
        if (results.count(identity) == 0)
            throw RuntimeException(
                    "Task [" + std::to_string(identity) + "] does not exist, the result may be fetched yet.");
    ...
    DBConnectionPool::DBConnectionPool(const string &hostName, int port, int threadNum, const string &userId,
        ...
        pool_ = new DBConnectionPoolImpl(hostName, port, threadNum, userId, password, loadBalance, highAvailability,
                                         compress, reConnect, python);
    }

    DBConnectionPool::~DBConnectionPool() {}

    void DBConnectionPool::run(const string &script, int identity, int priority, int parallelism, int fetchSize,
                               bool clearMemory) {
        if (identity < 0)
            throw RuntimeException(
                    "Invalid identity: " + std::to_string(identity) + ". Identity must be a non-negative integer.");
        pool_->run(script, identity, priority, parallelism, fetchSize, clearMemory);
    }

    void DBConnectionPool::run(const string &functionName, const vector<ConstantSP> &args, int identity, int priority,
    ...
    PartitionedTableAppender::PartitionedTableAppender(string dbUrl, string tableName, string partitionColName,
    ...
    PartitionedTableAppender::PartitionedTableAppender(string dbUrl, string tableName, string partitionColName,
                                                       string appendFunction, DBConnectionPool &pool) {
        pool_ = pool.pool_;
    ...
    void
    PartitionedTableAppender::init(string dbUrl, string tableName, string partitionColName, string appendFunction) {
    ...
    void PartitionedTableAppender::checkColumnType(int col, DATA_CATEGORY category, DATA_TYPE type) {
        if (columnTypes_[col] != type) {
            ...
            if (category != expectCategory) {
                throw RuntimeException("column " + std::to_string(col) + ", expect category " +
                                       Util::getCategoryString(expectCategory) + ", got category " +
                                       Util::getCategoryString(category));
    ...
    int
    SymbolBase::serialize(char *buf, int bufSize, INDEX indexStart, int offset, int &numElement, int &partial) const {
    ...
    RecordTime::RecordTime(const string &name) : name_(name) {
    ...
    std::string RecordTime::printAllTime() {
        ...
        for (std::unordered_map<std::string, RecordTime::Node *>::iterator iter = codeMap_.begin();
             iter != codeMap_.end(); iter++) {
        ...
        for (RecordTime::Node *node: nodes) {
            ...
            stdDev = sqrt(stdDev);
            output = output + node->name + ": sum = " + std::to_string(sum) + " count = " +
                     std::to_string(node->costTime.size()) +
