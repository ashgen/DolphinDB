...
namespace dolphindb {
    ...
    StreamDeserializer::StreamDeserializer(const unordered_map<string, pair<string, string>> &sym2tableName,
                                           DBConnection *pconn)
    ...
    void StreamDeserializer::create(DBConnection &conn) {
        ...
        for (auto &one: sym2tableName_) {
            if (one.second.first.empty()) {
                ...
                schema = conn.run(
                        std::string("schema(loadTable(\"") + one.second.first + "\",\"" + one.second.second + "\"))");
            }
    ...
    bool StreamDeserializer::parseBlob(const ConstantSP &src, vector<VectorSP> &rows, vector<string> &symbols,
        ...
        for (INDEX rowIndex = 0; rowIndex < rowSize; rowIndex++) {
            ...
            for (auto &colOne: cols) {
                ...
                if (colOne < ARRAY_TYPE_BASE) {
                    ...
                    if (ioError != OK) {
                        errorInfo.set(ErrorCodeInfo::EC_InvalidObject,
                                      "Deserialize blob error " + std::to_string(ioError));
                        return false;
                    }
    ...
    class StreamingClientImpl {
        ...
        struct SubscribeInfo {
            ...
            explicit SubscribeInfo(const string &host, int port, const string &tableName, const string &actionName,
                                   long long offset, bool resub,
        ...
        private:
            enum SubscriberRPCType {
                RPC_OK,
                RPC_START,
                RPC_END
            };
            enum SubscriberFromType {
                FROM_DDB,
                FROM_API
            };
            SocketSP socket_;
    ...
    public:
        ...
        MessageQueueSP subscribeInternal(const string &host, int port, const string &tableName,
                                         ...
                                         bool resubscribe = true, const VectorSP &filter = nullptr,
        ...
        void daemon() {
            ...
            while (isExit() == false) {
                try {
                    ...
                    ThreadSP t = new Thread(
                            new Executor(std::bind(&StreamingClientImpl::parseMessage, this, socket, publisher)));
                    t->start();
    ...
    void StreamingClientImpl::checkServerVersion(std::string host, int port) {
        ...
        if ((v0 == 2 && v1 == 0 && v2 >= 9) || (v0 == 2 && v1 == 10)) {
            ...
            if (listeningPort_ != 0) {
                DLogger::Warn(
                        "The server only supports subscription through reverse connection (connection initiated by the subscriber). The specified port will not take effect.");
            }
            listeningPort_ = 0;
        } else {
            ...
            if (listeningPort_ == 0) {
                throw RuntimeException(
                        "The server does not support subscription through reverse connection (connection initiated by the subscriber). Specify a valid port parameter.");
            }
        }
    }
                ...
                siteReconn_.op([&](unordered_map<string, pair<long long, long long>> &mp) {
                    for (auto &p: mp) {
                        ...
                        try {
                            ...
                            cerr << "#attempt= " << p.second.first
                                 << "activeClosePublishConnection on site got an exception "
                    ...
                    for (int i = 0; i < 3; ++i) {
                        ...
                        try {
                            ...
                            if (getNewLeader(e.what(), host, port)) {
                                cerr << "In reconnect: Got NotLeaderException, switch to leader node [" << host << ":"
                                     << port << "] for subscription" << endl;
                                HAStreamTableInfo haInfo{info.host, info.port, info.tableName, info.actionName, host,
                                                         port};
                ...
                while (!initResub_.empty() && isExit() == false) {
                    ...
                    try {
                        ...
                        if (getNewLeader(e.what(), _host, _port)) {
                            cerr << "when handle initResub_, Got NotLeaderException, switch to leader node [" << _host
                                 << ":" << _port << "] for subscription" << endl;
                            HAStreamTableInfo haInfo{info.host, info.port, info.tableName, info.actionName, _host,
                                                     _port};
                            haStreamTableInfo_.push_back(haInfo);
                            info.host = _host;
                            info.port = _port;
    ...
    string StreamingClientImpl::subscribeInternal(DBConnection &conn, SubscribeInfo &info) {
        ...
        if (!colLabels->isArray())
            throw RuntimeException("The publisher doesn't have the table [" + info.tableName + "].");
        ...
        if (isListenMode() == false) {
            std::shared_ptr<DBConnection> activeConn = std::make_shared<DBConnection>(false, false, 30, false, false,
                                                                                      true);
    ...
    MessageQueueSP StreamingClientImpl::subscribeInternal(const string &host, int port, const string &tableName,
        ...
        while (isExit() == false) {
            ++attempt;
            SubscribeInfo info(_host, _port, tableName, actionName, offset, resubscribe, filter, msgAsTable,
                               allowExists,
            ...
            try {
                ...
                if (attempt <= 10 && getNewLeader(e.what(), _host, _port)) {
                    cerr << "Got NotLeaderException, switch to leader node [" << _host << ":" << _port
                         << "] for subscription" << endl;
    ...
    void StreamingClientImpl::unsubscribeInternal(const string &host, int port, const string &tableName,
        ...
        auto iter1 = std::find_if(haStreamTableInfo_.begin(), haStreamTableInfo_.end(),
                                  [=](const HAStreamTableInfo &info) {
                                      return info.followIp == host && info.followPort == port &&
                                             info.tableName == tableName && info.action == actionName;
                                  });
        if (iter1 != haStreamTableInfo_.end()) {
            ...
            auto iter2 = std::find_if(haStreamTableInfo_.begin(), haStreamTableInfo_.end(),
                                      [=](const HAStreamTableInfo &info) {
                                          return info.leaderIp == host && info.leaderPort == port &&
                                                 info.tableName == tableName && info.action == actionName;
                                      });
    ...
    MessageQueueSP StreamingClient::subscribeInternal(string host, int port, string tableName, string actionName,
                                                      int64_t offset, bool resubscribe,
                                                      const dolphindb::VectorSP &filter,
    ...
    ThreadSP ThreadedClient::subscribe(string host, int port, const MessageBatchHandler &handler, string tableName,
                                       ...
                                       string userName, string password,
                                       const StreamDeserializerSP &blobDeserializer) {
        MessageQueueSP queue = subscribeInternal(std::move(host), port, std::move(tableName), std::move(actionName),
                                                 offset,
                                                 resub, filter, msgAsTable, allowExists, batchSize, userName, password,
                                                 blobDeserializer);
        if (queue.isNull()) {
    ...
    ThreadSP newHandleThread(const MessageHandler handler, MessageQueueSP queue, bool msgAsTable,
                             SmartPointer<StreamingClientImpl> impl) {
    ...
    ThreadSP ThreadedClient::subscribe(string host, int port, const MessageHandler &handler, string tableName,
        ...
        MessageQueueSP queue = subscribeInternal(std::move(host), port, std::move(tableName), std::move(actionName),
                                                 offset,
                                                 resub, filter, msgAsTable, false, 1, userName, password,
                                                 blobDeserializer);
        if (queue.isNull()) {
            cerr << "Subscription already made, handler loop not created." << endl;
            ThreadSP t = new Thread(new Executor([]() {}));
    ...
    MessageQueueSP PollingClient::subscribe(string host, int port, string tableName, string actionName, int64_t offset,
                                            ...
                                            string userName, string password,
                                            const StreamDeserializerSP &blobDeserializer) {
        return subscribeInternal(std::move(host), port, std::move(tableName), std::move(actionName), offset, resub,
                                 filter,
    ...
    vector<ThreadSP>
    ThreadPooledClient::subscribe(string host, int port, const MessageHandler &handler, string tableName,
                                  string actionName, int64_t offset, bool resub, const VectorSP &filter,
                                  bool msgAsTable, bool allowExists,
                                  string userName, string password,
                                  const StreamDeserializerSP &blobDeserializer) {
        auto queue = subscribeInternal(std::move(host), port, std::move(tableName), std::move(actionName), offset,
                                       resub,
    ...
    ThreadSP
    IPCInMemoryStreamClient::newHandleThread(const string &tableName, const IPCInMemoryTableReadHandler handler,
    ...
    SmartPointer<IPCInMemTable>
    loadIPCInMemoryTableAndCheckSchema(const string &tableName, const TableSP &outputTable, string &errMsg) {
    ...
    ThreadSP IPCInMemoryStreamClient::subscribe(const string &tableName, const IPCInMemoryTableReadHandler &handler,
                                                TableSP outputTable, bool overwrite) {
        if (tableName == "") {
