...
namespace dolphindb {
    ...
    ConstantSP StringVector::getSubVector(INDEX start, INDEX length, INDEX capacity) const {
        ...
        if (length > 0)
            ...
            vec->data_.insert(vec->data_.begin(), data_.rbegin() + (size() - 1 - start),
                              data_.rbegin() + (size() - 1 - start - length));
    ...
    int
    StringVector::serialize(char *buf, int bufSize, INDEX indexStart, int offset, int &numElement, int &partial) const {
        int size_ = size();
        if (indexStart >= size_)
            return -1;
        partial = 0;
    ...
    ConstantSP FastDateVector::castTemporal(DATA_TYPE expectType) {
        if (expectType != DT_DATEHOUR && (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP)) {
            throw RuntimeException(
                    "castTemporal from DATE to " + Util::getDataTypeString(expectType) + " not supported ");
        }
        if (expectType != DT_DATE && expectType != DT_TIMESTAMP && expectType != DT_NANOTIMESTAMP &&
            expectType != DT_MONTH && expectType != DT_DATETIME && expectType != DT_DATEHOUR) {
            throw RuntimeException(
                    "castTemporal from DATE to " + Util::getDataTypeString(expectType) + " not supported ");
        }
    ...
    ConstantSP FastDateTimeVector::castTemporal(DATA_TYPE expectType) {
        if (expectType != DT_DATEHOUR && (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP)) {
            throw RuntimeException(
                    "castTemporal from DATETIME to " + Util::getDataTypeString(expectType) + " not supported ");
        }
        ...
        if (expectType == DT_NANOTIMESTAMP || expectType == DT_TIMESTAMP) {
            ...
            for (int i = 0; i < size_; i++) {
                ...
                data_[i] == INT_MIN ? pbuf[i] = LLONG_MIN : pbuf[i] = (long long) (remainder +
                                                                                   ((data_[i] < 0) && remainder) *
                                                                                   86400) * 1000000000LL;
            }
        } else {
            ...
            if (ratio > 0) {
                for (int i = 0; i < size_; i++) {
                    int remainder = data_[i] % 86400;
                    data_[i] == INT_MIN ? pbuf[i] = INT_MIN : pbuf[i] = (remainder +
                                                                         ((data_[i] < 0) && remainder) * 86400) * ratio;
                }
            } else {
                ratio = -ratio;
                for (int i = 0; i < size_; i++) {
    ...
    ConstantSP FastDateHourVector::castTemporal(DATA_TYPE expectType) {
        if (expectType != DT_DATEHOUR && (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP)) {
            throw RuntimeException(
                    "castTemporal from DATEHOUR to " + Util::getDataTypeString(expectType) + " not supported ");
        }
        if (expectType == DT_DATEHOUR)
            return getValue();
        ...
        if (expectType == DT_NANOTIMESTAMP || expectType == DT_TIMESTAMP) {
            ...
            long long *pbuf = (long long *) res->getDataArray();
            for (int i = 0; i < size_; i++) {
                data_[i] == INT_MIN ? pbuf[i] = LLONG_MIN : pbuf[i] =
                                                                    (long long) data_[i] * 3600 % 86400 * 1000000000LL;
            }
        } else {
            ratio = Util::getTemporalConversionRatio(DT_SECOND, expectType);
            int *pbuf = (int *) res->getDataArray();
    ...
    ConstantSP FastTimeVector::castTemporal(DATA_TYPE expectType) {
        if (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP) {
            throw RuntimeException(
                    "castTemporal from TIME to " + Util::getDataTypeString(expectType) + " not supported ");
        }
    ...
    ConstantSP FastMinuteVector::castTemporal(DATA_TYPE expectType) {
        if (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP) {
            throw RuntimeException(
                    "castTemporal from MINUTE to " + Util::getDataTypeString(expectType) + " not supported ");
        }
        if (expectType != DT_TIME && expectType != DT_NANOTIME && expectType != DT_SECOND && expectType != DT_MINUTE) {
            throw RuntimeException(
                    "castTemporal from MINUTE to " + Util::getDataTypeString(expectType) + " not supported ");
    ...
    ConstantSP FastSecondVector::castTemporal(DATA_TYPE expectType) {
        if (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP) {
            throw RuntimeException(
                    "castTemporal from SECOND to " + Util::getDataTypeString(expectType) + " not supported ");
        }
        if (expectType != DT_TIME && expectType != DT_NANOTIME && expectType != DT_SECOND && expectType != DT_MINUTE) {
            throw RuntimeException(
                    "castTemporal from SECOND to " + Util::getDataTypeString(expectType) + " not supported ");
    ...
    ConstantSP FastNanoTimeVector::castTemporal(DATA_TYPE expectType) {
        if (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP) {
            throw RuntimeException(
                    "castTemporal from NANOTIME to " + Util::getDataTypeString(expectType) + " not supported ");
        }
        if (expectType != DT_TIME && expectType != DT_NANOTIME && expectType != DT_SECOND && expectType != DT_MINUTE) {
            throw RuntimeException(
                    "castTemporal from NANOTIME to " + Util::getDataTypeString(expectType) + " not supported ");
    ...
    ConstantSP FastTimestampVector::castTemporal(DATA_TYPE expectType) {
        if (expectType != DT_DATEHOUR && (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP)) {
            throw RuntimeException(
                    "castTemporal from TIMESTAMP to " + Util::getDataTypeString(expectType) + " not supported ");
        }
        ...
        if (expectType == DT_NANOTIMESTAMP) {
            ...
            for (int i = 0; i < size_; i++) {
                ...
                data_[i] == LLONG_MIN ? pbuf[i] = LLONG_MIN : pbuf[i] = (remainder +
                                                                         ((data_[i] < 0) && remainder) * 86400000) *
                                                                        1000000ll;
            }
        } else {
            ...
            for (int i = 0; i < size_; i++) {
                int remainder = data_[i] % 86400000;
                data_[i] == LLONG_MIN ? pbuf[i] = INT_MIN : pbuf[i] = (remainder +
                                                                       ((data_[i] < 0) && remainder) * 86400000) /
                                                                      ratio;
            }
        }
    ...
    ConstantSP FastNanoTimestampVector::castTemporal(DATA_TYPE expectType) {
        if (expectType != DT_DATEHOUR && (expectType < DT_DATE || expectType > DT_NANOTIMESTAMP)) {
            throw RuntimeException(
                    "castTemporal from NANOTIMESTAMP to " + Util::getDataTypeString(expectType) + " not supported ");
        }
        if (expectType == DT_NANOTIMESTAMP)
            return getValue();
        ...
        if (expectType == DT_TIMESTAMP) {
            ...
            for (int i = 0; i < size_; i++) {
                ...
                data_[i] == LLONG_MIN ? pbuf[i] = LLONG_MIN : pbuf[i] = (remainder + (data_[i] < 0 && remainder) *
                                                                                     86400000000000ll);
            }
        } else {
            ...
            for (int i = 0; i < size_; i++) {
                long long remainder = data_[i] % 86400000000000ll;
                data_[i] == LLONG_MIN ? pbuf[i] = INT_MIN : pbuf[i] = (remainder +
                                                                       (data_[i] < 0 && remainder) * 86400000000000ll) /
                                                                      ratio;
            }
        }
        return res;
    ...
    ConstantSP FastArrayVector::castTemporal(DATA_TYPE expectType) {
        if (expectType < ARRAY_TYPE_BASE) {
            throw RuntimeException("castTemporal from " + Util::getDataTypeString(dataType_) + " to " +
                                   Util::getDataTypeString(expectType) + " not supported.");
        }
    ...
    int FastArrayVector::serialize(char *buf, int bufSize, INDEX indexStart, int offset, int &numElement,
                                   int &partial) const {
    ...
    int
    FastArrayVector::serializeFixedLength(char *buf, int bufSize, INDEX indexStart, int offset, int targetNumElement,
        ...
        if (offset > 0) {
            ...
            bytesSent += value_->serialize(buf, bufSize, rowStart + offset, 0, cellCountToSerialize, tmpNumElement,
                                           tmpPartial);
    ...
    int
    FastArrayVector::serializeVariableLength(char *buf, int bufSize, INDEX indexStart, int offset, int targetNumElement,
                                             int &numElement, int &partial) const {
        return 0;
    }

    IO_ERR
    FastArrayVector::deserialize(DataInputStream *in, INDEX indexStart, INDEX targetNumElement, INDEX &numElement) {
        IO_ERR ret;
        if (baseUnitLength_ > 0)
    ...
    IO_ERR FastArrayVector::deserializeFixedLength(DataInputStream *in, INDEX indexStart, INDEX targetNumElement,
                                                   INDEX &numElement) {
        IO_ERR ret = OK;
        numElement = 0;
		...
		*/
            if (stage_ == 2) {
                ...
                if (tmpNumElement == valueCellCount) {
                    ...
                    INDEX index = lowerBoundIndex(pindex, indexStart + rowCount_ - rowsRead_, indexStart,
                                                  prevStart + tmpNumElement);
    ...
    IO_ERR FastArrayVector::deserializeVariableLength(DataInputStream *in, INDEX indexStart, INDEX targetNumElement,
                                                      INDEX &numElement) {
        return OK;
    }
    ...
    bool FastArrayVector::append(const ConstantSP &value) {
        if (value->isArray() && value->isTuple() == false &&
            ((Vector *) value.get())->getVectorType() != VECTOR_TYPE::ARRAYVECTOR) {
    ...
    ConstantSP FastBoolMatrix::getWindow(INDEX colStart, int colLength, INDEX rowStart, int rowLength) const {
        ...
        if (!colLabel_->isNull())
            matrix->setColumnLabel(((Vector *) colLabel_.get())->getSubVector(colStart, colLength));
        return matrix;
    }
    ...
    FastFixedLengthVector::FastFixedLengthVector(DATA_TYPE type, int fixedLength, int size, int capacity,
                                                 unsigned char *srcData, bool containNull) : fixedLength_(fixedLength),
                                                                                             size_(size),
                                                                                             capacity_(capacity),
                                                                                             type_(type),
                                                                                             containNull_(containNull) {
    ...
    IO_ERR FastFixedLengthVector::deserialize(DataInputStream *in, INDEX indexStart, INDEX targetNumElement,
                                              INDEX &numElement) {
        IO_ERR ret = OK;
    ...
    ConstantSP FastFixedLengthVector::getValue(INDEX capacity) const {
        ...
        Vector *copy = Util::createVector(type, size_, capacity, true, getExtraParamForType(), (void *) data,
                                          containNull_);
        copy->setForm(getForm());
        return copy;
    }
    ...
    int FastFixedLengthVector::serialize(char *buf, int bufSize, INDEX indexStart, int offset, int &numElement,
                                         int &partial) const {
    ...
    int FastFixedLengthVector::serialize(char *buf, int bufSize, INDEX indexStart, int offset, int cellCountToSerialize,
                                         int &numElement, int &partial) const {
        if (indexStart >= size_)
            return -1;
        int len = fixedLength_;
    ...
    const unsigned char *
    FastFixedLengthVector::getBinaryConst(INDEX start, int len, int unitLength, unsigned char *buf) const {
        return data_ + start * fixedLength_;
    }

    unsigned char *
    FastFixedLengthVector::getBinaryBuffer(INDEX start, int len, int unitLength, unsigned char *buf) const {
        return data_ + start * fixedLength_;
    }
    ...
    FastInt128Vector::FastInt128Vector(DATA_TYPE type, int size, int capacity, unsigned char *srcData, bool containNull)
            : FastRecordVector<Guid, GuidHash>(type, size, capacity, srcData, containNull) {
    }
    ...
    FastUuidVector::FastUuidVector(int size, int capacity, unsigned char *srcData, bool containNull) : FastInt128Vector(
            DT_UUID, size, capacity, srcData, containNull) {
    }

    FastIPAddrVector::FastIPAddrVector(int size, int capacity, unsigned char *srcData, bool containNull)
            : FastInt128Vector(DT_IP, size, capacity, srcData, containNull) {
    }
